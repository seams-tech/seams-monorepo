...
function extractSignedDelegateSenderId(signedDelegate: unknown): string {
  const delegateAction = readSignedDelegateActionFields(signedDelegate);
  return String(delegateAction?.senderId || '').trim() || 'unknown-sender';
}

type SignedDelegateActionFields = {
  readonly senderId: unknown;
  readonly receiverId: unknown;
  readonly maxBlockHeight: unknown;
};

function readSignedDelegateActionFields(value: unknown): SignedDelegateActionFields | null {
  if (!isPlainObject(value) || !isPlainObject(value.delegateAction)) return null;
  const delegateAction = value.delegateAction;
  return {
...
function parseSignedDelegateReplayDetails(value: string): {
  ...
  try {
    ...
    const execution = isPlainObject(parsed.execution) ? parsed.execution : null;
    return {
      execution: {
...
type NearExecutionOutcomeFields = {
  readonly status: unknown;
  readonly transactionOutcome: unknown;
  readonly receiptsOutcome: unknown;
  readonly finalExecutionStatus: unknown;
  readonly transaction: unknown;
};

function readNearExecutionOutcome(value: unknown): NearExecutionOutcomeFields | null {
  if (!isPlainObject(value)) return null;
  return {
    status: value.status,
    transactionOutcome: value.transaction_outcome,
    receiptsOutcome: value.receipts_outcome,
    finalExecutionStatus: value.final_execution_status,
    transaction: value.transaction,
  };
}

type NearOutcomeContainer = {
  readonly outcome: unknown;
};

function readNearOutcomeContainer(value: unknown): NearOutcomeContainer | null {
  if (!isPlainObject(value)) return null;
  return { outcome: value.outcome };
}

type NearExecutionRow = {
  readonly status: unknown;
  readonly tokensBurnt: unknown;
  readonly gasBurnt: unknown;
};

function readNearExecutionRow(value: unknown): NearExecutionRow | null {
  if (!isPlainObject(value)) return null;
  return {
    status: value.status,
    tokensBurnt: value.tokens_burnt,
    gasBurnt: value.gas_burnt,
  };
}
...
function resolveNearOutcomeFailure(outcome: unknown): {
  ...
  const executionOutcome = readNearExecutionOutcome(outcome);
  if (!executionOutcome) return null;
  const directChecks = [readNearFailureDetail(executionOutcome.status)];
  for (const failure of directChecks) {
    if (failure) return failure;
  }

  const transactionOutcome = readNearOutcomeContainer(executionOutcome.transactionOutcome);
  const rootOutcome = readNearExecutionRow(transactionOutcome?.outcome);
  const rootFailure = readNearFailureDetail(rootOutcome?.status);
  if (rootFailure) return rootFailure;

  if (Array.isArray(executionOutcome.receiptsOutcome)) {
    for (const entry of executionOutcome.receiptsOutcome) {
      const receipt = readNearOutcomeContainer(entry);
      const receiptOutcome = readNearExecutionRow(receipt?.outcome);
      const failure = readNearFailureDetail(receiptOutcome?.status);
      if (failure) return failure;
    }
  }

  return readNearFailureDetail(executionOutcome.finalExecutionStatus);
}
...
function resolveSignedDelegateAssessment(result: {
  ...
  const executionOutcome = readNearExecutionOutcome(result.outcome);
  const outcomeHash =
    executionOutcome && isPlainObject(executionOutcome.transaction)
      ? String(executionOutcome.transaction.hash || '').trim()
      : '';
...
function resolveThrownSignedDelegateAssessment(error: unknown): SignedDelegateExecutionAssessment {
  const row = isPlainObject(error) ? error : null;
...
function summarizeNearExecutionOutcome(outcome: unknown): {
  ...
  if (executionOutcome) {
    ...
    if (Array.isArray(executionOutcome.receiptsOutcome)) {
      for (const entry of executionOutcome.receiptsOutcome) {
        const receipt = readNearOutcomeContainer(entry);
        const receiptOutcome = readNearExecutionRow(receipt?.outcome);
        if (receiptOutcome) rows.push(receiptOutcome);
      }
    }
  }
  ...
  for (const row of rows) {
    const tokensBurntRaw = String(row.tokensBurnt || '').trim();
    if (tokensBurntRaw) {
      try {
        totalTokensBurnt += BigInt(tokensBurntRaw);
      } catch {}
    }
    const gasBurntRaw = row.gasBurnt;
...
function buildSignedDelegateDetailsJson(input: {
  ...
  const delegateAction = readSignedDelegateActionFields(input.signedDelegate);
  ...
  try {
    ...
    return await runSponsorshipExecution({
      ...
      onThrownError: async ({
        ...
        const errorRow = isPlainObject(error) ? error : null;
        let prepaidSettlement: {
          reservationId: string | null;
          settledAt: string;
          sourceEventId: string;
