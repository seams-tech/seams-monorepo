...
} from '@seams-internal/wallet-console-server/sponsoredCalls/index';
import {
...
} from '@seams-internal/wallet-console-server/runtimeSnapshots/index';
import {
...
} from '@seams-internal/console-server/enterpriseIsolation/index';
import {
...
} from '@seams-internal/console-server/account/index';
import {
  ...
  router.get('/console/audit/events', async (req: Request, res: Response) => {
    ...
    try {
      const request = parseListConsoleAuditEventsRequest((req as any).query || {}, [
        ...CONSOLE_AUDIT_QUERY_CATEGORIES,
        ...WALLET_CONSOLE_AUDIT_CATEGORIES,
      ]);
...
function registerConsoleWalletRoutes(router: ExpressRouter, ctx: ExpressConsoleContext): void {
  router.post('/console/wallets/balances/refresh', async (req: Request, res: Response) => {
    const claims = await requireConsoleAuth(req, res, ctx);
    if (!claims) return;
    if (!requireConsoleRoutePolicy(req, res, ctx, claims)) return;
    ...
    try {
      const request = parseRefreshConsoleWalletBalancesRequest((req as any).body || {});
      const result = await wallets.refreshBalances(toWalletContext(claims), request);
...
function registerConsoleWebhookRoutes(router: ExpressRouter, ctx: ExpressConsoleContext): void {
  ...
  router.post('/console/webhooks/:id/test', async (req: Request, res: Response) => {
    const claims = await requireConsoleAuth(req, res, ctx);
    if (!claims) return;
    if (!requireConsoleRoutePolicy(req, res, ctx, claims)) return;
    const webhooks = requireWebhookService(res, ctx);
    ...
    try {
      const result = await webhooks.sendTestEvent(toBillingContext(claims), endpointId);
      await emitConsoleAuditEvent(ctx, claims, {
        category: 'WEBHOOK',
