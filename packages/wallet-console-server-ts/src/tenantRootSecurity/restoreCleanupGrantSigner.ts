...
const GRANT_DOMAIN = new TextEncoder().encode('seams/tenant-root-restore-cleanup-grant/v1');
const GRANT_OPERATION = new TextEncoder().encode('tenant_root_restore_cleanup_v1');
const GRANT_AUTHENTICATION_DOMAIN = new TextEncoder().encode(
  'seams/tenant-root-restore-cleanup-grant/authentication/v1',
);
const ED25519_PKCS8_SEED_PREFIX = Uint8Array.from([
  0x30, 0x2e, 0x02, 0x01, 0x00, 0x30, 0x05, 0x06, 0x03, 0x2b, 0x65, 0x70, 0x04, 0x22, 0x04, 0x20,
]);
const RESTORE_CLEANUP_GRANT_NONCE_BYTES = 32;
const RESTORE_CLEANUP_GRANT_KEY_ID_MAX_BYTES = 256;

export const TENANT_ROOT_RESTORE_CLEANUP_GRANT_OPERATION_V1 = 'tenant_root_restore_cleanup_v1';
export const TENANT_ROOT_RESTORE_CLEANUP_GRANT_MAX_LIFETIME_MS_V1 = 300_000;

/** The exact public fields authenticated by one restore cleanup grant. */
export interface TenantRootRestoreCleanupGrantSigningInputV1 {
...
/** A signed grant and its public digest for the restore cleanup command. */
export interface SignedTenantRootRestoreCleanupGrantV1 {
  readonly destinationIdentityDigestB64u: string;
  readonly destinationFingerprintB64u: string;
  readonly destinationLineageB64u: string;
  readonly restoreSessionIdB64u: string;
...
function isNonzeroByte(byte: number): boolean {
  return byte !== 0;
}
...
function lengthPrefix(value: Uint8Array): Uint8Array {
  if (value.length === 0 || value.length > 0xffff_ffff) {
    throw new Error('Tenant-root restore cleanup grant canonical field length is invalid');
  }
...
function canonicalBase64UrlBytes(
  ...
  if (
    ...
    (requireNonzero && !bytes.some(isNonzeroByte))
  ) {
    throw new Error(`${label} is invalid`);
  }
...
async function sha256(bytes: Uint8Array): Promise<Uint8Array> {
  ...
  if (!subtle)
    throw new Error('WebCrypto is required for tenant-root restore cleanup grant issuance');
...
function validateTimeWindow(issuedAtMs: number, expiresAtMs: number): void {
  if (
    ...
    expiresAtMs - issuedAtMs > TENANT_ROOT_RESTORE_CLEANUP_GRANT_MAX_LIFETIME_MS_V1
  ) {
    throw new Error('Tenant-root restore cleanup grant time window is invalid');
  }
}
...
/** Signs the native `TenantRootRestoreCleanupGrantV1` wire exactly. */
export async function signTenantRootRestoreCleanupGrantV1(
  input: TenantRootRestoreCleanupGrantSigningInputV1,
): Promise<SignedTenantRootRestoreCleanupGrantV1> {
  ...
  const nonce = canonicalBase64UrlBytes(
    input.nonceB64u,
    RESTORE_CLEANUP_GRANT_NONCE_BYTES,
    'nonceB64u',
    true,
  );
  const grantKeyId = textBytes(
    input.grantKeyId,
    'grantKeyId',
    RESTORE_CLEANUP_GRANT_KEY_ID_MAX_BYTES,
  );
  ...
  try {
    ...
    if (signature.length !== 64) {
      throw new Error('Tenant-root restore cleanup grant authority produced an invalid signature');
    }
