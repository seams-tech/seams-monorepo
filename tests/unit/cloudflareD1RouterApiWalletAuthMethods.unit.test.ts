  ...
  try {
    ...
    const service = createCloudflareD1RouterApiAuthService({
      ...
      tenantRootCustodyLineage: {
        ...FIXTURE_TENANT_ROOT_CUSTODY_LINEAGE,
        async resolveActiveLineageForRuntimeScope(identity) {
    ...
    if (!intent.ok) throw new Error(intent.message);
    const runtimePolicyScope = normalizeRuntimePolicyScope(intent.intent.runtimePolicyScope);
    const reorderedIntent: typeof intent.intent = {
      nonceB64u: intent.intent.nonceB64u,
      runtimePolicyScope: intent.intent.runtimePolicyScope,
      signerSelection: intent.intent.signerSelection,
      walletId: intent.intent.walletId,
    ...
    await expect(
      service.walletAuthMethods.finalizeWalletAddSigner({
        ...finalizeRequest,
        ecdsa: { expectedKeyHandles: ['drifted-key-handle'] },
      }),
