...
test('pending registration parser accepts activation and deferred NEAR records', () => {
  ...
  const wasmOptionalFields = parsePendingWalletRegistrationCommitV1(
    pendingRecord({
      localMaterial: {
        keyFamilies: ['ed25519'],
        custodyCommit: {
  ...
  expect(wasmOptionalFields).not.toBeNull();
  expect(wasmOptionalFields?.localMaterial.custodyCommit).toEqual(custodyCommit);
