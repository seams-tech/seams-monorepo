...
function authIdentityWebAuthnAuthentication(): WebAuthnAuthenticationCredential {
  return {
    id: 'credential:auth-identity-step-up',
    rawId: 'credential:auth-identity-step-up',
    type: 'public-key',
...
function unlinkRequest(): Request {
  return new Request('https://relay.example.test/auth/unlink', {
    ...
    body: JSON.stringify({
      ...
      webauthn_authentication: authIdentityWebAuthnAuthentication(),
    }),
  });
}
