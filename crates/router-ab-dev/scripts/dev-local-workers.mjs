...
import { strictWorkerSourceDigest } from './strict-worker-source-digest.mjs';

import { prepareRouterAbD1LocalRuntimeConfig } from './d1-local-runtime-config.mjs';
import { prepareRouterAbStrictLocalRuntimeConfigs } from '../../router-ab-cloudflare/scripts/prepare-local-runtime-config.mjs';

const repoRoot = fileURLToPath(new URL('../../..', import.meta.url));

const STRICT_BUILD_RECEIPT_SCHEMA_VERSION = 'router_ab_strict_local_build_v5';
...
try {
  ...
  d1Runtime = prepareD1LocalRouterConfig();
  ...
  try {
    ...
    console.log(`${error.message} Rebuilding Worker artifacts automatically…`);
    buildProductionWorkerBinaries();
    assertProductionWorkerBinariesReady();
  }
    ...
    `${JSON.stringify(
      {
        ...
        roles,
        source_digest: strictWorkerSourceDigest(repoRoot),
      },
      null,
      2,
    )}\n`,
...
function assertProductionWorkerBinariesReady() {
  ...
  if (missingArtifacts.length > 0) {
    throw new Error(
      `Router A/B Worker artifacts are missing: ${missingArtifacts.join(', ')}`,
    );
  }
  if (!existsSync(strictBuildReceiptPath)) {
    throw new Error(
      'Router A/B Worker build receipt is missing.',
    );
  }
  ...
  try {
    ...
    throw new Error(
      'Router A/B Worker build receipt is invalid.',
    );
  }
  ...
  if (
    ...
    throw new Error(
      `Router A/B Worker artifacts need a build receipt for the current inputs and ${strictWorkerBuildProfile} profile.`,
    );
  }
  if (receipt.source_digest !== strictWorkerSourceDigest(repoRoot)) {
    throw new Error(
      'Router A/B Worker build inputs have changed since the last build.',
    );
  }
}
...
async function ensureGateway() {
  ...
  const child = spawn('pnpm', ['run', 'gateway:server'], {
    ...
    env: {
      ...
      SEAMS_LOCAL_CONSOLE_ORG_ID: d1Runtime.localConsoleOrganizationId,
      ROUTER_AB_INTERNAL_SERVICE_AUTH_SECRET:
