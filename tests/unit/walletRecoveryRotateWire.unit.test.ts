...
test('the client refuses a rotation response with unsupported fields', async () => {
  const result = await rotateWalletRecoverySet({
    relayUrl: 'https://relay.localhost',
    walletId: WALLET_ID,
    factorProof: FACTOR_PROOF,
  ...
  expect(result).toEqual({
    ...
    message: 'rotation failed (HTTP 200)',
  });
});

test('the client refuses a recovery-set response with unsupported fields', async () => {
  const result = await readWalletRecoverySet({
    relayUrl: 'https://relay.localhost',
    walletId: WALLET_ID,
    factorProof: FACTOR_PROOF,
    fetchImpl: respondWith(200, {
  ...
  expect(result).toEqual({
    kind: 'transport_failed',
    message: 'recovery set read returned an unusable payload',
  });
});
