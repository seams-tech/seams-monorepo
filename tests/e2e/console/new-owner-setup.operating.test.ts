...
test.describe('Console operating paths', () => {
  test('new owner setup creates a publishable key that reveals once and survives reload', async ({
    ...
    await expect(navigation.getByRole('link', { name: 'API Keys', exact: true })).toHaveAttribute('aria-disabled', 'true');
    ...
    await expect(page.getByRole('heading', { name: 'Onboarding complete', exact: true })).toHaveCount(0);
...
test('an account without an organization leaves login for dashboard onboarding', async ({ page }) => {
  ...
  await expect(page).toHaveURL(/\/dashboard\/onboarding\/?$/);
  await expect(page.getByRole('main', { name: 'Dashboard onboarding' })).toBeVisible();
  await expect(page.getByRole('list', { name: 'Onboarding progress' })).toContainText('Organization');
  await expect(page.getByRole('list', { name: 'Onboarding progress' })).toContainText('Project');
  await expect(page.getByRole('button', { name: 'Continue to project setup', exact: true })).toBeVisible();
