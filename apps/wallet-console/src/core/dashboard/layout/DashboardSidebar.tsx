...
const MOBILE_NAVIGATION_QUERY = '(max-width: 1160px)';

function subscribeMobileNavigation(callback: () => void): () => void {
  const query = window.matchMedia(MOBILE_NAVIGATION_QUERY);
  query.addEventListener('change', callback);
  return removeMobileNavigationListener.bind(null, query, callback);
}

function removeMobileNavigationListener(query: MediaQueryList, callback: () => void): void {
...
function serverMobileNavigation(): boolean {
  return false;
}

export function openMobileNavigation(): void {
  const dialog = document.getElementById('dashboard-mobile-navigation');
...
function dismissMobileNavigationLink(event: React.MouseEvent<HTMLDialogElement>): void {
  if (
  ...
  if (link && link.getAttribute('aria-disabled') !== 'true') closeMobileNavigation();
}

function selectMobileAccountOption(onSelect: (value: string) => void, option: TopbarOption): void {
  if (option.keepMenuOpen !== true) closeMobileNavigation();
...
export function DashboardSidebar<Route extends string, GroupKey extends string>({
  ...
  workspace,
  homeProps,
}: DashboardSidebarProps<Route, GroupKey>): React.JSX.Element {
  ...
  const content = (
    ...
    >
      ...
      {mobile ? (
        <div className="dashboard-mobile-support">
          <a href={getDocsOrigin()} target="_blank" rel="noreferrer">
            Documentation <span aria-hidden="true">↗</span>
          </a>
          <details className="dashboard-mobile-account">
            <summary>
            ...
            <div>
              {accountOptions.map((option) => (
                <button
                  ...
                  onClick={selectMobileAccountOption.bind(null, onSelectAccount, option)}
                >
                  {option.label}
                </button>
              ))}
  ...
  return mobile ? (
    <dialog
      ...
      onClick={dismissMobileNavigationLink}
    >
      {content}
    </dialog>
  ) : (
