...
const deploymentIdentity = resolveDeploymentIdentity(deploymentComponent, apply);
...
if (manifestFile) {
  ...
  if (!deploymentComponent) {
    throw new Error('--manifest-file requires --component wallet-system or --component site');
  }
  ...
  if (deploymentComponent === 'wallet-system') {
  ...
  if (json) {
    process.stdout.write(`${JSON.stringify({ ...preparedManifest, application }, null, 2)}\n`);
  } else {
...
const preparation = prepare
  ? writePreparedComponentManifestFor(output, deploymentComponent)
  : undefined;
if (preparation) {
  progress.detail(`Saved ${deploymentComponent} manifest to ${preparation.manifestPath}`);
}

if (json) {
  process.stdout.write(`${JSON.stringify({ ...completenessOutput, preparation }, null, 2)}\n`);
} else {
  printHumanOutput(completenessOutput, undefined);
...
Usage:
  pnpm wallet-system:deploy:env-prepare -- --lane staging-testnet
  pnpm wallet-system:deploy:env-prepare -- --lane production-mainnet --rotate
  pnpm wallet-system:deploy:env-apply -- --lane staging-testnet --manifest-file <path>
  pnpm site:deploy:env-apply -- --site staging --manifest-file <path>

Options:
  --lane <id>                  Backend lane for wallet-system.
  --site <id>                  Frontend site: staging or production.
  ...
  --prepare                    Provision infrastructure and write one protected authority manifest.
  --manifest-file <path>       Prepared wallet-system or site manifest.
  --component <name>           wallet-system or site.
...
Prepare mode writes one protected authority manifest under ~/.seams/backups.
Wallet-system and site generations are independent.

WARNING: Every prepare invocation generates fresh identities. Preparing or
applying wallet-system for an initialized target requires --rotate.`);
}
...
function assertNoLegacyIdentityFlags() {
  ...
  if (legacyFlag) {
    throw new Error(`${legacyFlag} is retired; use --lane for wallet-system or --site for site`);
  }
}
...
function resolveDeploymentIdentity(component, requiresProvisionedLane) {
  ...
  const resolvedComponent = component || (laneId ? 'wallet-system' : siteId ? 'site' : undefined);
  if (!resolvedComponent) {
    throw new Error('--lane or --site is required');
  }
  if (resolvedComponent === 'wallet-system') {
    if (!laneId) {
      throw new Error('--lane is required for wallet-system operations');
    }
    if (siteId) {
      throw new Error('--site cannot be used for wallet-system operations');
    }
  ...
  if (!siteId) {
    throw new Error('--site is required for site operations');
  }
  if (laneId) {
    throw new Error('--lane cannot be used for site operations');
  }
...
function buildOutput(input) {
  ...
  return {
    ...
    siteLaneHandoffs:
      input.siteId === 'production' ? buildSiteLaneHandoffs(deploymentInput) : undefined,
    environments,
...
function buildSiteLaneHandoffs(input) {
  return Object.fromEntries(
    input.siteLanes.map((lane) => [
      lane.network,
      {
        ...
        generationId: manual(`production-${lane.network}-wallet-system-generation-id`),
...
function buildDeploymentDigestPayload(output) {
  return {
    ...
    siteLaneHandoffs: output.siteLaneHandoffs,
  ...
  return createHash('sha256')
    .update(
      JSON.stringify({
        ...
        siteLaneHandoffs: manifest.siteLaneHandoffs,
...
function buildGeneralEnvironment(input) {
  ...
  if (input.siteId === 'production') {
    return [environmentName, buildProductionSiteEnvironment(input)];
  }
  ...
  return [
    environmentName,
    {
      ...
      secrets: {
        ...
        SEAMS_WALLET_ARTIFACT_READ_TOKEN: manual(
...
function buildProductionSiteEnvironment(input) {
  const variables = {};
  ...
  return [
    environmentName,
    {
      ...
      variables: {
        ...
        ...backup.variables,
        ...(readBackendLane(input.laneId).network === 'mainnet'
          ? {
              ROUTER_AB_DERIVER_A_TENANT_ROOT_RECOVERY_KMS_KEY_RING: manual(
                `${environmentName}-tenant-root-recovery-kms-key-ring`,
              ),
            }
          : {}),
        ROUTER_AB_TENANT_ROOT_CONTROL_PLANE_ISSUER_VERIFYING_KEYS_JSON:
  ...
  return [
    environmentName,
    {
      ...
      variables: {
        ...
        ...(readBackendLane(input.laneId).network === 'mainnet'
          ? {
              ROUTER_AB_DERIVER_B_TENANT_ROOT_RECOVERY_KMS_KEY_RING: manual(
                `${environmentName}-tenant-root-recovery-kms-key-ring`,
              ),
            }
          : {}),
...
function buildTenantRootControlPlaneEnvironment(input) {
  ...
  return [
    environmentName,
    {
      ...
      variables: {
        ...(readBackendLane(input.laneId).id === 'production-mainnet' ||
        readBackendLane(input.laneId).id === 'staging-testnet'
          ? {
              ROUTER_AB_TENANT_ROOT_RECOVERY_TRUST_BUNDLE_JSON: manual(
                `${environmentName}-tenant-root-recovery-trust-bundle-json`,
              ),
              ROUTER_AB_TENANT_ROOT_RECOVERY_TRUST_SNAPSHOT_JSON: manual(
                `${environmentName}-tenant-root-recovery-trust-snapshot-json`,
              ),
            }
          : {}),
        ROUTER_AB_DERIVER_A_TENANT_ROOT_CREATION_SIGNING_KEY_ID:
...
function assertCompleteApplyInput(outputDocument, shouldApply, incompleteAllowed, valuesFilePath) {
  ...
  if (outputDocument.deploymentComponent) {
    ...
    valuesFileInstruction =
      ...
      'deployment/wallet-system/env/deployment-values.example.env, fill it, and rerun.';
  }
  throw new Error(
...
function validateGatewayRegistrationDocuments(outputDocument) {
  ...
  assertGeneratedValueEqual(ceremonyJwk.kty, 'OKP', 'ceremony JWT JWK kty');
  assertGeneratedValueEqual(ceremonyJwk.crv, 'Ed25519', 'ceremony JWT JWK curve');
  ...
  assertGeneratedValueEqual(ceremonyJwks.keys.length, 1, 'ceremony JWT public JWKS key count');
  const ceremonyPublicJwk = ceremonyJwks.keys[0];
  assertGeneratedValueEqual(ceremonyPublicJwk.kty, ceremonyJwk.kty, 'ceremony JWT public JWK kty');
  assertGeneratedValueEqual(
    ceremonyPublicJwk.crv,
    ceremonyJwk.crv,
    'ceremony JWT public JWK curve',
  );
  assertGeneratedValueEqual(ceremonyPublicJwk.x, ceremonyJwk.x, 'ceremony JWT public key');
  assertGeneratedValueEqual(
...
function assertExactKeys(value, expectedKeys, label) {
  assertOrderedKeyNamesEqual(Object.keys(value), expectedKeys, `${label} fields`);
}

function assertGeneratedValueEqual(actual, expected, label) {
  if (actual !== expected) {
    throw new Error(`${label} mismatch: ${JSON.stringify(actual)} != ${JSON.stringify(expected)}`);
  }
}

function assertOrderedKeyNamesEqual(actual, expected, label) {
  if (
...
function writePreparedComponentManifestFor(output, component) {
  const backupDirectory = join(homedir(), '.seams', 'backups');
  mkdirSync(backupDirectory, { recursive: true, mode: 0o700 });
  chmodSync(backupDirectory, 0o700);
  const manifest = buildPreparedComponentManifest(output, component);
  const manifestPath = writePreparedComponentManifest(backupDirectory, manifest);
  return {
    generationId: output.generationId,
    manifestSha256: output.manifestSha256,
    component,
...
function writePreparedComponentManifest(backupDirectory, manifest) {
  const identityId =
    manifest.deploymentComponent === 'wallet-system' ? manifest.lane : manifest.site;
...
function validatePreparedComponentManifest(manifest, identity, component) {
  ...
  assertGeneratedValueEqual(
    ...
    1,
    'prepared deployment manifest schema version',
  );
  assertGeneratedValueEqual(manifest.target, identity.release, 'prepared deployment release');
  assertGeneratedValueEqual(
  ...
  assertGeneratedValueEqual(
    manifest[component === 'wallet-system' ? 'lane' : 'site'],
    identity.id,
    'prepared deployment identity',
  );
  assertGeneratedValueEqual(
    manifest.deploymentComponent,
    component,
    'prepared deployment component',
  );
  parseStrictGatewayDeploymentConfig(
...
function readDeploymentComponent() {
  ...
  if (component && !VALID_DEPLOYMENT_COMPONENTS.has(component)) {
    throw new Error('--component must be wallet-system or site');
  }
