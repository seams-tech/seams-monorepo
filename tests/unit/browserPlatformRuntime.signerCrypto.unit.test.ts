...
test.describe('browser SignerCryptoPort ECDSA bootstrap', () => {
  ...
  test('parses exact generated ECDSA bootstrap command envelopes', () => {
    ...
    expect(parseGeneratedPrepareEcdsaClientBootstrapCommand(prepareCommand)).toEqual({
      ...prepareCommand,
      participants: {
        ...prepareCommand.participants,
        participantIds: [1, 2],
      },
    });
    expect(parseGeneratedFinalizeEcdsaClientBootstrapCommand(finalizeCommand)).toEqual(
      finalizeCommand,
    );
    expect(() =>
      parseGeneratedPrepareEcdsaClientBootstrapCommand({ ...prepareCommand, extra: true }),
    ).toThrow();
    expect(() =>
      parseGeneratedFinalizeEcdsaClientBootstrapCommand({
        ...finalizeCommand,
        relayerPublicIdentity: {
          ...finalizeCommand.relayerPublicIdentity,
          extra: true,
        },
      }),
    ).toThrow();
  ...
  test('rejects runtime ECDSA worker handles carrying a provisioning slot', () => {
    expect(() =>
      parseEmailOtpWorkerIssuedSessionHandle({
        kind: 'email_otp_worker_session_handle_v1',
        sessionId: 'otp-session',
