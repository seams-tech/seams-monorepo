...
const IMPORT_PATHS = {
  ...
  confirmCommon:
    '/_test-sdk/esm/core/signingEngine/stepUpConfirmation/channel/confirmCommon.js',
  ...
  ecdsaSealedRuntime: '/_test-sdk/esm/core/signingEngine/session/material/ecdsaSealedRuntime.js',
...
async function installPasskeyRestoreBrowserState(input: {
  ...
  browserAuthorization: Awaited<ReturnType<typeof buildPasskeyRestoreBrowserAuthorizationFixture>>;
}): Promise<void> {
  ...
  const exactSession =
    await indexedDbMod.walletSessionAuthorizations.readExactWithOperationCredential({
...
test.describe('UserConfirm worker router', () => {
  ...
  test('routes pending references while preserving callback identity and state ownership', async ({
    page,
  }) => {
    const result = await page.evaluate(
      async ({ paths }) => {
        const managerMod = await import(paths.touchConfirmManager);
        const common = await import(paths.confirmCommon);
        const manager = managerMod.createUiConfirmManager(
          {},
          {
            touchIdPrompt: {},
            nearClient: {},
        ...
        const posted: any[] = [];
        const onResend = () => Promise.resolve();
        const emailOtpPrompt = { challengeId: 'challenge-1', onResend };
        const request = {
        ...
        const fakeWorker: Worker = {
          ...
          postMessage: ((message: any) => {
            posted.push(message);
            if (message?.type !== 'SECURE_CONFIRM_REQUEST') return;
            const pending = (manager as any).pendingConfirmRequests.get(request.requestId);
            const prompt = {
              type: 'PROMPT_USER_CONFIRM_IN_JS_MAIN_THREAD',
              requestId: request.requestId,
              channelToken: 'channel-token-1',
              data: {
            ...
            const normalize = (manager as any).normalizePromptEnvelope.bind(manager);
            const resolved = normalize(prompt);
            const missingId = normalize({
              ...prompt,
              requestId: '',
              data: { ...prompt.data, requestId: '' },
            });
            const spoofedToken = normalize({
              ...prompt,
              data: { ...prompt.data, requestToken: 'worker-token-spoofed' },
            });
            const fullRequest = normalize({
              ...prompt,
              data: { ...prompt.data, request },
            });
            observation = {
              resolvedSameRequest: resolved?.data === request,
              callbackIdentityPreserved: resolved?.data.payload.emailOtpPrompt.onResend === onResend,
              missingIdIgnored: missingId === null,
              spoofedTokenIgnored: spoofedToken === null,
              fullRequestIgnored: fullRequest === null,
              promptTokenMatchesMessageId: pending.requestToken === message.id,
            };
            const responsePort = common.createUserConfirmScopedWorker(fakeWorker, {
              channelToken: prompt.channelToken,
              onDecision: (manager as any).capturePendingConfirmationDecision.bind(
                manager,
                pending,
              ),
            });
            common.sendConfirmResponse(responsePort, {
              requestId: request.requestId,
              confirmed: true,
            ...
            for (const listener of listeners) {
              listener({
                data: { id: message.id, success: true, data: { ok: true } },
                currentTarget: fakeWorker,
                target: fakeWorker,
              });
            }
        ...
        (manager as any).worker = fakeWorker;
        (manager as any).attachWorkerRouter(fakeWorker);
        const decision = await (manager as any).requestUserConfirmation(request);

        return {
          ...observation,
          decisionCaptured: decision?.confirmed === true,
          responseStrippedHandle: !('_confirmHandle' in (posted[1]?.data ?? {})),
          sentPayload: posted[0]?.payload,
        };
      },
      { paths: IMPORT_PATHS },
    );
    ...
    expect(result).toEqual({
      resolvedSameRequest: true,
      callbackIdentityPreserved: true,
      missingIdIgnored: true,
      spoofedTokenIgnored: true,
      fullRequestIgnored: true,
      decisionCaptured: true,
      promptTokenMatchesMessageId: true,
      responseStrippedHandle: true,
      sentPayload: { requestId: 'reference-1' },
    });
  });

  test('uses the worker request token and ignores a stale callback on request id reuse', async ({
    page,
  }) => {
    const result = await page.evaluate(
      async ({ paths }) => {
        ...
        let staleCapture: ((decision: any) => void) | undefined;
        let phase = 0;
        const fakeWorker: Worker = {
          addEventListener: ((type: string, handler: (event: any) => void) => {
            if (type === 'message') listeners.push(handler);
          ...
          postMessage: ((message: any) => {
            posted.push(message);
            if (message?.type !== 'SECURE_CONFIRM_REQUEST') return;
            const pending = (manager as any).pendingConfirmRequests.get(
              message.payload.requestId,
            );
            if (phase === 0) {
              firstPending = pending;
              staleCapture = (manager as any).capturePendingConfirmationDecision.bind(
                manager,
                pending,
              );
              phase = 1;
            } else {
              staleCapture?.({ requestId: message.payload.requestId, confirmed: true });
            }
            for (const listener of listeners) {
              listener({
                data: { id: message.id, success: true, data: { ok: true } },
        ...
        const secondRequest = {
          ...
          summary: { walletId: 'wallet-1' },
          payload: { walletId: 'wallet-1' },
        };
        const firstError = await (manager as any).requestUserConfirmation(firstRequest).then(
          () => '',
          (error: any) => String(error?.message || error),
        );
        const firstToken = firstPending?.requestToken;
        const secondError = await (manager as any).requestUserConfirmation(secondRequest).then(
          () => '',
          (error: any) => String(error?.message || error),
        );

        return {
          sentPayload: posted.map((message) => message.payload),
          sentTokens: posted.map((message) => message.id),
          firstToken,
          firstError,
          secondError,
          pendingAfter: (manager as any).pendingConfirmRequests.size,
        };
      },
      { paths: IMPORT_PATHS },
    );

    expect(result.sentPayload).toEqual([
      { requestId: 'reused-request-id' },
      { requestId: 'reused-request-id' },
    ]);
    expect(result.sentTokens).toHaveLength(2);
    expect(result.sentTokens[0]).toBe(result.firstToken);
    expect(result.sentTokens[0]).not.toBe(result.sentTokens[1]);
    expect(result.firstError).toContain('missing host decision');
    expect(result.secondError).toContain('missing host decision');
    expect(result.pendingAfter).toBe(0);
  });
  ...
  test('exportPrivateKeysWithUi rejects completed worker result with an error', async ({
    page,
  }) => {
    const result = await page.evaluate(
      async ({ paths }) => {
        ...
        emitMessage({
          ...
          data: {
            ...
            accountId: 'alice.testnet',
            exportedSchemes: [],
            error: 'completed result cannot include an error',
          },
        });
