...
const projectEnvironmentId = 'local-smoke-project:dev';
let publishableKey = '';
const docsOrigin = process.env.SEAMS_INTENDED_DOCS_ORIGIN || 'https://docs.localhost:4003';
const walletTestAppViteUrl = 'http://127.0.0.1:4004';
...
const walletTestAppViteCacheDir =
...
let consoleSessionCookie = '';
const consoleFixturePath = path.join(routerAbLocalRoot, 'console-fixture.json');
let webServerReadyServer;
const localConsoleOrganizationId = `org_${randomBytes(6).toString('hex')}`;
...
async function main() {
  ...
  createConsoleTestFixture();
  await createLocalTenantRoot();

  const walletTestApp = startWalletTestApp();
  await waitForHttpOk(walletTestAppViteUrl, 'Wallet test app Vite', 120_000);
  const consoleApp = startConsole();
  await waitForHttpOk(appUrl, 'Wallet test app', 120_000);
  ...
  await waitForWalletTestAppModuleGraphArtifacts();
  await waitForHttpOk(intendedPageSmokeUrl(), 'intended page', 60_000);
  await waitForRouterStability();
  await startWebServerReadyServer();

  console.log('[intended-services] Console, Wallet test app, and Router are ready');
  await waitUntilStopped(walletTestApp, router, consoleApp);
}
...
function resetLocalState() {
  ...
  removeAbsolutePath(walletTestAppViteCacheDir);
}
...
function startWalletTestApp() {
  return spawnManaged(
    'wallet-test-app',
    ['exec', 'vite', '--config', 'tests/intended-app/vite.config.ts'],
    walletTestAppEnv(),
  );
}
...
function createConsoleTestFixture() {
  console.log('[intended-services] creating isolated workspace through Console APIs');
  const result = spawnSync('pnpm', ['-C', 'tests', 'seed:intended-local-console'], {
    cwd: repoRoot,
    env: {
      ...process.env,
      SEAMS_INTENDED_ROUTER_URL: routerUrl,
      SEAMS_INTENDED_APP_URL: appUrl,
      SEAMS_INTENDED_WALLET_ORIGIN: walletOrigin,
      SEAMS_INTENDED_CONSOLE_FIXTURE_PATH: consoleFixturePath,
  ...
  if (result.error) {
    throw new Error(`Console fixture setup failed to start: ${result.error.message}`);
  }
  if (result.status !== 0) {
    throw new Error(`Console fixture setup exited with ${String(result.status ?? 'unknown')}`);
  }
  const fixture = JSON.parse(readFileSync(consoleFixturePath, 'utf8'));
  if (
    typeof fixture.cookie !== 'string' ||
    !fixture.cookie ||
    typeof fixture.publishableKey !== 'string' ||
    !fixture.publishableKey
  )
    throw new Error('Invalid Console fixture response');
  consoleSessionCookie = fixture.cookie;
...
function localConsoleAuthHeaders() {
  if (!consoleSessionCookie) throw new Error('Console setup has not issued a session');
  return { Cookie: consoleSessionCookie };
}
...
async function waitForWalletTestAppModuleGraphArtifacts() {
  for (const relativePath of requiredWalletTestAppModuleGraphArtifacts) {
    await waitForHttpOk(
      walletTestAppModuleGraphUrl(relativePath),
      `sdk module ${relativePath}`,
      60_000,
    );
  }
}
...
function walletTestAppModuleGraphUrl(relativePath) {
  const absolutePath = path.join(repoRoot, relativePath);
...
function isSiteViteCommand(command) {
  return (
    command.includes('tests/intended-app/vite.config.ts') &&
