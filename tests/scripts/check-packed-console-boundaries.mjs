...
const publicWalletPackages = [{ dir: 'packages/wallet-server', name: '@seams/wallet-server' }];
...
for (const pkg of [...consoleCorePackages, ...walletConsolePackages, ...publicWalletPackages]) {
...
// Composed workspace: Console core, Wallet Console, and Wallet server all from
// tarballs. No source checkout participates in module resolution.
...
copyIntoNodeModules(
  composedNodeModules,
  '@seams/wallet-server',
  unpacked.get('@seams/wallet-server'),
);
