...
test.describe('device-linking key worker', () => {
  ...
  test('zeroizes transferred ordinary material while preserving idempotent worker-owned state', async () => {
    ...
    scope.responses.shift();
    const reorderedTargetFactor: DeviceLinkingOrdinaryTargetFactorBindingV1 = {
      credentialIdB64u: targetFactor.credentialIdB64u,
      rpId: targetFactor.rpId,
      verificationDigestB64u: targetFactor.verificationDigestB64u,
      walletAuthMethodId: targetFactor.walletAuthMethodId,
      kind: targetFactor.kind,
    };
    scope.send({
      id: 'prepare-insertion-order-replay',
      request: privateRequest(
        expectedFactorSecret.slice(),
        recipientPrivateKey.slice(),
        reorderedTargetFactor,
      ),
    });
    const reorderedPreparation = await waitForResponse(scope);
    expect(reorderedPreparation).toMatchObject({ ok: true });
    expect(reorderedPreparation.result).toEqual(firstPreparation.result);
    scope.responses.shift();
    ...
    scope.responses.shift();
    const reorderedCommitted = {
      packageSetDigestB64u: committed.packageSetDigestB64u,
      authority: committed.authority,
      authMethod: committed.authMethod,
      signerPackages: committed.signerPackages,
      ed25519ExportRootPackage: committed.ed25519ExportRootPackage,
      kind: committed.kind,
    };
    scope.send({
      id: 'seal-insertion-order-replay',
      request: { ...sealRequest, committed: reorderedCommitted },
    });
    const reorderedSealed = await waitForResponse(scope);
    expect(reorderedSealed).toMatchObject({ ok: true });
    expect(reorderedSealed.result).toEqual(sealed.result);
    expect(sealCalls).toBe(1);
    scope.responses.shift();
    scope.send({
    ...
    scope.responses.shift();
    const changedCommitted = {
      ...committed,
      authority: {
        ...committed.authority,
        updatedAtMs: committed.authority.updatedAtMs + 1,
      },
    };
    scope.send({
      id: 'seal-conflicting-nested-replay',
      request: { ...sealRequest, committed: changedCommitted },
    });
    const conflictingNestedSeal = await waitForResponse(scope);
    expect(conflictingNestedSeal).toMatchObject({
      ok: false,
      error: 'ordinary signer material seal conflicts with the existing result',
    });
    scope.responses.shift();
    const conflictingFactorBranch: DeviceLinkingOrdinaryTargetFactorBindingV1 = {
      kind: 'email_otp',
      walletAuthMethodId,
      verificationDigestB64u: packageSetDigestB64u,
      emailHashHex: 'f'.repeat(64),
      registrationAuthorityId: 'authority:worker-duplicate',
    };
    scope.send({
      id: 'seal-conflicting-branch-replay',
      request: { ...sealRequest, targetFactor: conflictingFactorBranch },
    });
    const conflictingBranchSeal = await waitForResponse(scope);
    expect(conflictingBranchSeal).toMatchObject({
      ok: false,
      error: 'ordinary signer material target factor binding changed',
    });
    scope.responses.shift();
