...
import {
  ...
  fetchJson,
  getPath,
  makeConsoleAuthAdapter as makeStaticConsoleAuthAdapter,
  startExpressRouter,
} from './helpers';
import type {
...
function makeConsoleAuthAdapter(
  ...
  return makeStaticConsoleAuthAdapter(makeConsoleAuthClaims(input, orgId, userId));
}
...
test.describe('console router (express)', () => {
  ...
  test('GET /console/observability/* returns scaffolded responses when service is configured', async () => {
    ...
    const router = createConsoleRouter({
      auth: makeConsoleAuthAdapter(
        CONSOLE_AUTH_PLATFORM_SUPPORT_OWNER,
  ...
  test('Stripe webhook processing failures emit processing observability events (express)', async () => {
    ...
    try {
      ...
      const res = await fetchJson(`${srv.baseUrl}/console/billing/stripe/webhook`, {
        ...
        headers: {
          'Content-Type': 'application/json',
          'Stripe-Signature': stripeSignatureHeader('whsec_expected_processing_express', rawBody),
  ...
  test('POST /console/projects auto-provisions environments with live environments disabled without billing readiness', async () => {
    ...
    const router = createConsoleRouter({
      auth: makeConsoleAuthAdapter(
        CONSOLE_AUTH_OWNER,
        'org-env-billing-gate',
        'user-env-billing-gate',
      ),
      orgProjectEnv,
      billing,
    });
  ...
  test('POST /console/projects enables live environments when billing bypass is explicitly enabled', async () => {
    ...
    const router = createConsoleRouter({
      auth: makeConsoleAuthAdapter(
        CONSOLE_AUTH_OWNER,
        'org-env-billing-bypass',
        'user-env-billing-bypass',
      ),
      orgProjectEnv,
  ...
  test('approval queue mutations emit approval lifecycle webhook events when webhook endpoint is configured', async () => {
    ...
    const router = createConsoleRouter({
      auth: makeConsoleAuthAdapter(
        CONSOLE_AUTH_OWNER,
        'org-approval-events-1',
        'user-approval-events-1',
      ),
      approvals,
      webhooks,
    });
  ...
  test('new console endpoints support scaffold CRUD flows', async () => {
    ...
    const router = createConsoleRouter({
      auth: makeConsoleAuthAdapter(
        CONSOLE_AUTH_OWNER,
        'org-scaffold-express-1',
        'user-scaffold-express-1',
      ),
      policies,
      keyExports,
      runtimeSnapshots,
  ...
  test('new console endpoints enforce org isolation', async () => {
    ...
    const ownerRouter = createConsoleRouter({
      auth: makeConsoleAuthAdapter(
        CONSOLE_AUTH_OWNER,
        ownerOrgId,
        'owner-scaffold-express-isolation-user',
      ),
      policies,
      keyExports,
      runtimeSnapshots,
  ...
  test('wallet routes support list/search/detail', async () => {
    ...
    const router = createConsoleRouter({
      auth: makeConsoleAuthAdapter(
        CONSOLE_AUTH_OWNER,
        'org-wallet-express-1',
        'user-wallet-express-1',
      ),
      wallets,
    });
    const srv = await startExpressRouter(router);
  ...
  test('policy create rejects client-supplied ids', async () => {
    ...
    const router = createConsoleRouter({
      auth: makeConsoleAuthAdapter(
        CONSOLE_AUTH_OWNER,
        'org-policy-create-id-validation',
        'user-policy-id',
      ),
      policies,
    });
  ...
  test('webhook mutations require console config mutation role', async () => {
    ...
    try {
      ...
      const forbiddenTest = await fetchJson(
        `${developerServer.baseUrl}/console/webhooks/${encodeURIComponent(endpointId)}/test`,
        { method: 'POST' },
      );
      expect(forbiddenTest.status).toBe(403);
      const sentTest = await fetchJson(
        `${adminServer.baseUrl}/console/webhooks/${encodeURIComponent(endpointId)}/test`,
        { method: 'POST' },
      );
      expect(sentTest.status).toBe(200);
      expect(getPath(sentTest.json, 'result', 'attempted')).toBe(1);
  ...
  test('administrator with billing permissions can access billing read routes', async () => {
    ...
    const router = createConsoleRouter({
      auth: makeConsoleAuthAdapter(
        CONSOLE_AUTH_BILLING_MANAGER,
        'org-1',
        'user-billing-manager-read-1',
      ),
      billing,
    });
    const srv = await startExpressRouter(router);
...
test.describe('console router (cloudflare adaptor)', () => {
  test('serves the shared Fetch route surface', async () => {
    const handler = createCloudflareConsoleRouter({ healthz: true });
    const health = await callCf(handler, { method: 'GET', path: '/console/healthz' });
    expect(health.status).toBe(200);
    expect(health.json?.service).toBe('console');

    const relayPath = await callCf(handler, {
      method: 'POST',
  ...
  test('rejects Postgres tenant routes in the Cloudflare runtime', async () => {
    ...
    expect(response.json?.code).toBe('tenant_storage_backend_not_supported_in_cloudflare_runtime');
