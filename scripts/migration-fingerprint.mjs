...
export function assertAppliedMigrationSourcesUnchanged({
  ...
  // Older deploys could record a migration row before checkpointing its prefix.
  const firstUnappliedIndex = migrations.findIndex(
    (migration) => !appliedMigrationNames.has(migration.name),
  );
  const appliedAfterGap =
    firstUnappliedIndex !== -1 &&
    migrations.slice(firstUnappliedIndex).some((migration) =>
      appliedMigrationNames.has(migration.name),
    );
  if (!appliedAfterGap) {
    for (let prefixLength = 0; prefixLength < previouslyAppliedMigrations.length; prefixLength += 1) {
      if (digestMigrations(migrations.slice(0, prefixLength)) === previousFingerprint) return;
    }
  }
