...
const COMPONENTS = new Set(['wallet-system', 'site']);
...
const WALLET_SYSTEM_CLOUDFLARE_ENVIRONMENT_SUFFIXES = Object.freeze([
  '-gateway',
  '-mpc-router',
  '-deriver-a',
  '-deriver-b',
...
async function main(options) {
  ...
  if (options.component === 'site') {
    if (options.site.id === 'staging') {
      addSiteNearRelayerUpdate(plan, values);
      addSiteNearNetworkUpdates(plan);
    }
  } else {
    addWalletSystemNearRelayerSecretUpdate(plan, values);
  }
  validatePlan(plan);
...
function buildBasePlan(options, repository, values) {
  ...
  if (options.component === 'site') {
    if (options.site.id === 'production') {
      appendProductionSiteUpdates(plan, values);
    } else {
    ...
    appendWalletSystemCloudflareDeploymentUpdates(plan, options.environmentPrefix, values);
    appendMappedUpdates(
      plan.secrets,
...
function appendWalletSystemCloudflareDeploymentUpdates(plan, target, values) {
  for (const suffix of WALLET_SYSTEM_CLOUDFLARE_ENVIRONMENT_SUFFIXES) {
...
function addSiteNearRelayerUpdate(plan, values) {
...
function addSiteNearNetworkUpdates(plan) {
...
function addWalletSystemNearRelayerSecretUpdate(plan, values) {
  ...
  if (!plan.gatewayDeploymentConfig.optional.nearRelayer) {
    throw new Error(
      'RELAYER_PRIVATE_KEY requires nearRelayer in deployment/wallet-system/targets.json',
    );
  }
  plan.secrets.push({
...
function validateCheckedInGatewayConfiguration(config, values) {
  ...
  if (walletOrigin && !config.origins.allowedCors.includes(walletOrigin)) {
    throw new Error(
      'VITE_WALLET_ORIGIN must be updated in deployment/wallet-system/targets.json first',
    );
  }
...
function printPlan(plan, applying) {
  ...
  process.stdout.write(
    `${plan.component === 'wallet-system' ? 'Lane' : 'Site'}: ${plan.identityId} (release ${plan.release})\n\n`,
  );
...
Usage:
  pnpm wallet-system:deploy:env-update -- --lane staging-testnet --repo seams-tech/seams-monorepo
  pnpm site:deploy:env-update -- --site staging --repo seams-tech/seams-monorepo

Options:
  --lane <id>           Required for wallet-system.
  --site <id>           Required for site: staging or production.
  --component <name>    Required. wallet-system or site.
