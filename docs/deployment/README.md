...
deployment/wallet-system/targets.json
  release sites, Wallet backend lanes, resources, provisioning state, and
  non-secret Gateway/Router configuration
```
...
- Required secret names are derived from the owning target and component.
  Preflight reads only the explicitly bound environment values for that job.
...
Deriver A and Deriver B secrets never share a job. Each deployment job binds
one owning environment and performs its component preflight from explicit
inputs. The existing `staging` and `production` environments own the frontend
