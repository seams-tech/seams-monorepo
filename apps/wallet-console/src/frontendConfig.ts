...
export async function loadFrontendConfig(source: ImportMetaEnv): Promise<FrontendConfig> {
  return hydrateSiteBindings(buildSiteConfig(source));
}

export async function loadWalletHostConfig(
  source: ImportMetaEnv,
  origin: string,
): Promise<SeamsConfigsInput> {
  const site = buildSiteConfig(source);
  const candidates = Object.values(site.deployments);
  const deployment = candidates.find((candidate) => candidate.walletOrigin === origin);
  if (!deployment) throw new Error('This origin is not configured as a hosted wallet.');
  return buildSeamsSdkConfig(await hydrateDeploymentBinding(deployment, origin));
}
