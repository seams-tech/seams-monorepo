...
class D1ConsoleWebhookServiceImpl implements ConsoleWebhookD1Service {
  ...
  async emitEvent(
    ...
    return this.deliverEvent(ctx, { eventId, eventType, payload: request.payload }, endpoints);
  }
  ...
  async sendTestEvent(
    ...
    const endpoint = await requireEndpoint({ state: this.state, orgId: ctx.orgId, endpointId });
    ...
    return this.deliverEvent(ctx, createWebhookTestEvent(endpointId, this.state.now()), [endpoint]);
  }

  private async deliverEvent(
    ctx: ConsoleWebhooksContext,
    request: Required<EmitConsoleWebhookEventRequest>,
    endpoints: StoredWebhookEndpoint[],
  ): Promise<EmitConsoleWebhookEventResult> {
