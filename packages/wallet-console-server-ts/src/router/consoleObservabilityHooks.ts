...
import { buildWalletApprovalFailureObservabilityEvent } from '../observability';
...
export function readConsoleRequestTraceContext(
...
export function readConsoleStripeWebhookFailureMetadata(raw: unknown): {
  ...
  const orgId = normalizeString(object.client_reference_id) || normalizeString(metadata.org_id);
...
export async function emitConsoleApprovalFailureObservabilityEvent(
  ...
  const event = buildWalletApprovalFailureObservabilityEvent({
    orgId: input.claims.orgId,
    ...(input.claims.projectId ? { projectId: input.claims.projectId } : {}),
    ...(input.claims.environmentId ? { environmentId: input.claims.environmentId } : {}),
