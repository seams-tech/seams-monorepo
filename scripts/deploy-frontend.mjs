...
import { consoleOriginFor } from '../packages/wallet-console-server-ts/scripts/gateway-deployment-config.mjs';
...
function printPlan(site) {
  const lines = [
    ...
    '',
    'Order:',
    '  1. build the Wallet SDK, marketing site, and Console app',
    '  2. consume the exact public Wallet docs artifact',
    '  3. mount the Console build at /dashboard-static and deploy the site Pages project',
    '  4. deploy the docs Pages project and bind its custom domain',
    '  5. deploy one wallet Pages project for every declared network',
    '  6. smoke site, Console routes, docs, SDK, and every Wallet origin',
  ];
...
function buildFrontend(site) {
  ...
  assertFile(path.join(SITE_OUTPUT, 'docs', 'index.html'), 'Wallet docs entry');
...
function copyPublicDocsArtifact() {
  ...
  copyDirectory(PUBLIC_DOCS_ARTIFACT_ROOT, DOCS_OUTPUT);
  copyDirectory(PUBLIC_DOCS_ARTIFACT_ROOT, path.join(DOCS_OUTPUT, 'docs'));
  copyDirectory(PUBLIC_DOCS_ARTIFACT_ROOT, path.join(SITE_OUTPUT, 'docs'));
}

function isMainnetLane(lane) {
  return lane.network === 'mainnet';
}
...
function buildFrontendEnvironment(site) {
  ...
  const consoleLane = site.id === 'production'
    ? site.lanes.find(isMainnetLane)
    : site.lanes[0];
  if (!consoleLane) throw new Error(`Missing Console deployment lane for ${site.id}`);
  const consoleOrigin = site.id === 'production'
    ? consoleOriginFor(consoleLane.gatewayOrigin)
    : consoleLane.gatewayOrigin;
  for (const lane of site.lanes) {
    ...
    // Console identity is shared across the site’s wallet networks.
    environment[`${prefix}CONSOLE_BASE_URL`] = consoleOrigin;
