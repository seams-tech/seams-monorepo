...
function reorderReplayEnvelope(
  envelope: ReplayCoreFixture['targetEnvelope'],
): ReplayCoreFixture['targetEnvelope'] {
  return {
...
function replayEnvelopeWithUpdatedAt(
  envelope: ReplayCoreFixture['targetEnvelope'],
  updatedAtMs: number,
): ReplayCoreFixture['targetEnvelope'] {
  return {
...
async function replay(
  ...
  replacementEnvelope: ReplayCoreFixture['targetEnvelope'] = fixture.targetEnvelope,
) {
  return await resolveCommittedRecoveryReplayV1({
    ...
    replacementId: String(replacementEnvelope.envelopeId),
    replacementEnvelope,
  });
}
...
test('replay compares replacement envelope fields independently of insertion order', async () => {
  const fixture = await buildReplayFixture(true);
  const reorderedEnvelope = reorderReplayEnvelope(fixture.targetEnvelope);

  await expect(replay(fixture, reorderedEnvelope)).resolves.toMatchObject({ kind: 'promoted' });

  await expect(
    replay(
      fixture,
      replayEnvelopeWithUpdatedAt(reorderedEnvelope, reorderedEnvelope.updatedAtMs + 1),
    ),
  ).resolves.toEqual({
    kind: 'rejected',
