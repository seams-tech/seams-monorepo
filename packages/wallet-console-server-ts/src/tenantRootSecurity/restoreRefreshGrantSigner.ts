...
const GRANT_DOMAIN = new TextEncoder().encode('seams/tenant-root-restore-refresh-grant/v1');
const GRANT_OPERATION = new TextEncoder().encode('tenant_root_restore_refresh_v1');
const GRANT_AUTHENTICATION_DOMAIN = new TextEncoder().encode(
  'seams/tenant-root-restore-refresh-grant/authentication/v1',
);
const ED25519_PKCS8_SEED_PREFIX = Uint8Array.from([
  0x30, 0x2e, 0x02, 0x01, 0x00, 0x30, 0x05, 0x06, 0x03, 0x2b, 0x65, 0x70, 0x04, 0x22, 0x04, 0x20,
]);
const RESTORE_REFRESH_GRANT_NONCE_BYTES = 32;
const RESTORE_REFRESH_GRANT_KEY_ID_MAX_BYTES = 256;

export const TENANT_ROOT_RESTORE_REFRESH_GRANT_OPERATION_V1 = 'tenant_root_restore_refresh_v1';
export const TENANT_ROOT_RESTORE_REFRESH_GRANT_MAX_LIFETIME_MS_V1 = 300_000;

/** The exact public fields authenticated by one restore refresh grant. */
export interface TenantRootRestoreRefreshGrantSigningInputV1 {
  ...
  readonly deriverAAcceptanceReceiptDigestB64u: string;
  readonly deriverBAcceptanceReceiptDigestB64u: string;
...
/** A signed grant and its public digest for the restore refresh command. */
export interface SignedTenantRootRestoreRefreshGrantV1 {
  readonly operationDigestB64u: string;
  readonly destinationIdentityDigestB64u: string;
  readonly destinationFingerprintB64u: string;
  readonly destinationLineageB64u: string;
...
function lengthPrefix(value: Uint8Array): Uint8Array {
  if (value.length === 0 || value.length > 0xffff_ffff) {
    throw new Error('Tenant-root restore refresh grant canonical field length is invalid');
  }
...
async function sha256(bytes: Uint8Array): Promise<Uint8Array> {
  ...
  if (!subtle)
    throw new Error('WebCrypto is required for tenant-root restore refresh grant issuance');
...
function unsignedGrantBytes(input: {
  ...
  readonly deriverAAcceptanceReceiptDigest: Uint8Array;
  readonly deriverBAcceptanceReceiptDigest: Uint8Array;
  ...
  return concatBytes([
    ...
    lengthPrefix(input.deriverAAcceptanceReceiptDigest),
    lengthPrefix(input.deriverBAcceptanceReceiptDigest),
...
/** Signs the native `TenantRootRestoreRefreshGrantV1` wire exactly. */
export async function signTenantRootRestoreRefreshGrantV1(
  input: TenantRootRestoreRefreshGrantSigningInputV1,
  ...
  const deriverAAcceptanceReceiptDigest = canonicalBase64UrlBytes(
    input.deriverAAcceptanceReceiptDigestB64u,
    32,
    'deriverAAcceptanceReceiptDigestB64u',
    true,
  );
  const deriverBAcceptanceReceiptDigest = canonicalBase64UrlBytes(
    input.deriverBAcceptanceReceiptDigestB64u,
    32,
    'deriverBAcceptanceReceiptDigestB64u',
    true,
  );
  if (equalBytes(deriverAAcceptanceReceiptDigest, deriverBAcceptanceReceiptDigest)) {
    throw new Error('Deriver acceptance receipt digests must differ');
  }
  const nonce = canonicalBase64UrlBytes(
    input.nonceB64u,
    RESTORE_REFRESH_GRANT_NONCE_BYTES,
    'nonceB64u',
    true,
  );
  const grantKeyId = textBytes(
    input.grantKeyId,
    'grantKeyId',
    RESTORE_REFRESH_GRANT_KEY_ID_MAX_BYTES,
  );
  ...
  try {
    ...
    if (signature.length !== 64) {
      throw new Error('Tenant-root restore refresh grant authority produced an invalid signature');
    }
    ...
    return {
      ...
      deriverAAcceptanceReceiptDigestB64u: input.deriverAAcceptanceReceiptDigestB64u,
      deriverBAcceptanceReceiptDigestB64u: input.deriverBAcceptanceReceiptDigestB64u,
