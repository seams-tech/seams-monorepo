# Refactor 105C: Unified Wallet Site And Console

Date created: August 21, 2026

Last reconciled: September 12, 2026 (unified wallet origin and public docs route)

Status: planned. This is private `seams-monorepo` work and begins after
Refactors 105B and 99B complete.

Preparatory R105B work has established separate Console and Wallet-system
target files, GitHub environments, manifests, environment commands, and deploy
workflows. The Console pipeline now owns its session, OAuth, email, webhook,
billing, grant-authority, Worker, and D1 values. Wallet-system manifests no
longer carry those values. The temporary production-testnet Console target
remains until this plan merges both production network data sets into the one
`production-console` authority. The shared internal-service credential also
stays exclusively inside Wallet-system environments; Console reaches the exact
tenant-root operations through its private `WALLET_RUNTIME` control port.

## Decision

Deploy one wallet site and same-origin customer Console API:

```text
production   https://wallet.seams.sh
staging      https://wallet.staging.seams.sh
```

The site serves Wallet marketing at `/`, the customer dashboard at
`/dashboard/*`, public Wallet VitePress documentation at `/docs/*`, and the
Console Worker at `/console/*`. Marketing, dashboard, and docs form one Pages
artifact and release. The Console Worker remains a separately deployed backend.

[Refactor 123](./refactor-123-domains.md) owns DNS, frontend publication,
OAuth/origin changes, redirects, and route cutover. R105C owns the private
Console source, customer data, authority, target configuration, and deployment
pipeline consumed by that cutover. The repository extraction and hostname
cutover remain separate changes.
...
Refactor 120 is complete. Its tenant-root runtime, role stores, and issuer and
wrapping keys stay inside the Wallet system. R121's Wallet Console product owns
redacted tenant-root identity, lineage, epoch, operation and restore state,
approvals, and audit records. Console core owns none of those domain models.
Deployment handoff remains service identity and version only; tenant-specific
metadata comes from authenticated product operations and persisted records.

The R105C data cutover must include R121 journals, step-up/approval records,
recovery/restore records, active-lineage lookup, and scheduler ownership.
Preserve exact tenant/environment/network scope and operation replay state.
Before switching authorities, drain in-flight mutations or resume their durable
journals through one canonical owner, and disable the old scheduler. Moving
the Console must never produce two rotation dispatchers for the same root.

Console grant-signing keys authorize exact Wallet operations and remain
distinct from the Wallet control-plane issuer key. R105 replaces R121's broad
Router binding/shared internal secret with a narrow control port. The canonical
Console cutover must consume that port and configure its step-up RP ID as
`wallet.seams.sh` with the exact origin. Console has no existing passkeys;
initial enrollment starts there without credential migration or re-enrollment.
...
dashboard. They do not receive separate customer login domains. Staging has its
...
```text
seams.sh
  company site and product discovery

docs.seams.sh
  private operational documentation, where still required

wallet.seams.sh
  wallet product landing page
  public Wallet documentation at /docs/*
  production dashboard at /dashboard/*
  Console Worker at /console/*
  production testnet and mainnet Wallet administration

wallet.staging.seams.sh
  staging wallet frontend and Console Worker
...
The wallet frontend owns `/`, `/dashboard/*`, and the assembled `/docs/*`
artifact. The same-origin Console Worker owns `/console/*`, `/healthz`, and
`/readyz`. Dispatch `/console/*` before static files or SPA fallback. Redirect
`/docs` to `/docs/`, serve `/docs/*` from VitePress output, then exact frontend
assets and the wallet SPA.
Wallet Gateway, hosted Wallet, Console, and Admin keep separate route and
...
Move the current `seams.sh/wallet` content into this composition. Public Wallet
documentation source and VitePress configuration live in `seams-wallet`; the
private deployment consumes its exact versioned static artifact.
...
`deployment/console/targets.json` is the single human-edited source for the
wallet frontend and Console
topology. Each hosted stage declares:

- the wallet-site, company-site, private-docs, Gateway, and iframe origins;
...
- preserve company-site and private-docs deployment values under their private
  owners; they do not enter the Wallet-system pipeline. The public Wallet docs
  artifact is an input to the Console-owned wallet frontend publication and
  carries no secret-write authority.
...
- [ ] Classify the current paired generator inputs and outputs, GitHub
      ...
      Console/wallet-site, Wallet-system, or company/private-docs.
...
- [ ] Add the site-level Console deployment configuration and explicit Wallet
      service targets.
- [ ] Add the exact public VitePress artifact input and assemble it at `/docs/`
      in the shared wallet Pages output.
...
- [ ] Confirm the landing page and a direct `/docs/*` deep link are public, and
      `/console/*` errors never fall through to frontend HTML.
...
Exit: staging uses only `wallet.staging.seams.sh` and its exact testnet target.
...
Exit: `wallet.seams.sh` is the only production Console authority.

### Phase 4: Remove The Old Topology

- [ ] Remove the Console bundle and `/dashboard/*` routing from `seams.sh`.
- [ ] Remove the old `seams.sh/wallet` publication after its content and links
      resolve at `wallet.seams.sh/`.
...
- [x] Delete the paired `wallet-core`/`product` generator path, combined
...
- production and staging use only `wallet.seams.sh` and
  `wallet.staging.seams.sh`, respectively;
...
- `seams.sh` contains no Wallet landing page, customer Console bundle, public
  Wallet docs, or dashboard route;
- one wallet Pages artifact publishes marketing, dashboard, and the exact
  public VitePress artifact at `/docs/`; public routes have no dashboard auth
  gate or global `SeamsWebProvider`;
- `/console/*` reaches the Console Worker before `/docs/*`, static assets, and
  SPA fallback;
...
- Wallet Console preserves R121 public metadata and durable operation state,
  with one scheduler and no tenant-root secrets or broad participant binding;
