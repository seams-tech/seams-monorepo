...
test('D1 staging runbook renders exact Phase 6 command sequence from readiness-clean configs', async () => {
  ...
  expect(markdown).toContain(
    'pnpm --dir packages/wallet-console-server-ts run d1:staging:resources',
  );
  expect(markdown).toContain(
    'pnpm --dir packages/wallet-console-server-ts run d1:staging:migrate',
  );
  expect(markdown).toContain(
  ...
  expect(markdown).toContain(
    'pnpm --dir packages/wallet-console-server-ts run d1:staging:reconcile',
  );
  expect(markdown).toContain(
