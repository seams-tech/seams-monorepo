...
} from '../../packages/wallet-server/src/router/domains/ed25519Yao/capabilityLifecycle/routerAbEd25519YaoProductRegistrationPartitioning';
import {
...
test.describe('Ed25519 Yao request-scoped persistence boundary', () => {
  ...
  test('partitions ceremony records without isolating shared replay and capability indexes', () => {
    ...
    const lifecycleSessions = new Map(partition.ceremony.registration.lifecycleSessions);
    lifecycleSessions.set('ceremony-a', 'session-a-updated');
    const merged = mergeRouterAbEd25519YaoProductRegistrationStatePartitionV1(state, {
      kind: partition.kind,
      lifecycleId: partition.lifecycleId,
      shared: partition.shared,
      ceremony: {
        kind: partition.ceremony.kind,
        lifecycleId: partition.ceremony.lifecycleId,
        registration: {
          states: partition.ceremony.registration.states,
          lifecycleSessions,
          admissionClaims: partition.ceremony.registration.admissionClaims,
        },
        authorization: partition.ceremony.authorization,
        recovery: partition.ceremony.recovery,
        export: partition.ceremony.export,
      },
    });
