...
test.describe('linked-device committed delivery continuation', () => {
  test('decodes only exact plain-object durable resume records', async () => {
    const fixture = await buildResumeFixture('resume-boundary');
    const resume = buildDeviceLinkingCommittedResumeV1({
    ...
    expect(parseDeviceLinkingCommittedResumeV1(resume)).toEqual(resume);
    expect(parseDeviceLinkingCommittedResumeV1({ ...resume, unexpected: true })).toBeNull();
    expect(parseDeviceLinkingCommittedResumeV1(Object.assign(new Date(0), resume))).toBeNull();
    ...
    const acknowledgement: LocalAuthorityActivationFinalAckV1 = {
      ...
      installationReceiptDigestB64u: fixture.active.sealedDelivery.installationReceiptDigestB64u,
      acknowledgedAtMs: 500,
    };
    const pending = buildDeviceLinkingPendingAcknowledgementV1(acknowledgement);
    expect(parseDeviceLinkingPendingAcknowledgementV1(pending)).toEqual(pending);
    expect(parseDeviceLinkingPendingAcknowledgementV1({ ...pending, unexpected: true })).toBeNull();
    expect(
      parseDeviceLinkingPendingAcknowledgementV1(Object.assign(new Date(0), pending)),
    ).toBeNull();
  });
