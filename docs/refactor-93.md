...
The completed deletion audit is retained in Git history.
...
- [x] Reconcile the frozen staging revision's admitted signing-root scope with
      ...
      the July 26 staging custody-rotation record in Git history.
...
The Gateway persistence replacement is complete. Its retained contract is
request-scoped composition over partitioned D1 records: batch-snapshot reads,
typed claims before non-idempotent work, atomic compare-and-swap transitions,
and exact replay through terminal receipts. Handlers must not share mutable
tenant snapshots across requests. Hosted recovery acceptance remains tracked
by this plan; the retired follow-up did not establish that acceptance.
The adapter history follows.
