...
- [refactor-120-rotate-tenant-secrets.md](./refactor-120-rotate-tenant-secrets.md)
  for tenant-root creation and operational-share refresh;
- [refactor-121-tenant-derivation-root-security.md](./refactor-121-tenant-derivation-root-security.md)
  for tenant-root recovery controls and exact-root disaster recovery;
...
identity-preserving transition supported by the current curve-specific protocol;
an envelope rewrite can never silently change its public key or address.
...
the current curve-specific signing protocol. Refactor 104 owns agent-key custody
and any optional delegated-execution holder package; it cannot reuse passkey custody
...
The current Router A/B protocols own key-family material provisioning.
Refactor 103E owns the linked-device authority and product behavior.
...
- Suspected plaintext exposure requires the affected signing material's
  curve-specific refresh or rekey protocol. R120 tenant-root share refresh
  leaves active signing material unchanged.
...
(`walletRecoveryPromotion.ts`, `/wallets/recovery/promote`,
`walletRecoveryPromote.ts`). Promotion's safety property is its ordering: the
new envelope is created before any old one is retired, so a failed create
leaves the wallet openable by the credential being replaced rather than by
nothing at all. Spending needed no new store: each manifest KEK wrap already carries its own lifecycle, so a
