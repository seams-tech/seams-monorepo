...
import { parseIsoTimestamp, type IsoTimestamp } from '@shared/utils/canonicalPrimitives';
import {
...
async function exercisePreparedActivationCancellation(input: {
  ...
  readonly conflictingCreatedAt: IsoTimestamp;
}) {
  await new Promise<void>((resolve) => {
    const request = indexedDB.deleteDatabase('seams_wallet');
    request.onsuccess = () => resolve();
  ...
  const conflictingJournal = {
    kind: preparation.journal.kind,
    journalId: preparation.journal.journalId,
    expectedManifest: preparation.journal.expectedManifest,
    activationCommand: preparation.journal.activationCommand,
    candidate: preparation.journal.candidate,
    createdAt: input.conflictingCreatedAt,
  };
...
test.describe('canonical ECDSA capability manifest store', () => {
  ...
  test('requires wallet custody rejoin for a server-committed activation after reload', async ({
    page,
  }) => {
    const fixture = ecdsaCapabilityActivationFixture();
    await prepareStoreModulePage(page);
    ...
    const result = await page.evaluate(
      async ({ storeModule, workerUrl, fixture }) => {
        ...
        return {
          ...
          journal: journal.kind === 'found' ? journal.journal.kind : journal.kind,
          active: active.kind,
        };
      },
    ...
    expect(result).toEqual({
      ...
      journal: 'server_activation_committed',
      active: 'missing',
    });
  });
  ...
  test('atomically removes only the exact prepared journal and its sealing key', async ({
    ...
    const result = await page.evaluate(exercisePreparedActivationCancellation, {
      storeModule: STORE_MODULE,
      fixture,
      conflictingCreatedAt: parseIsoTimestamp('2025-01-02T00:00:00.000Z'),
    });
