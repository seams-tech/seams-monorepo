...
import { localPeerVerifyingKeyHex } from '../../router-ab-cloudflare/scripts/local-key-material.mjs';
import { resolveLocalTenantRootKeyMaterial } from '../../router-ab-cloudflare/scripts/prepare-local-runtime-config.mjs';
...
export function prepareRouterAbD1LocalRuntimeConfig(input) {
  ...
  const sourceConfigPath = path.resolve(
    input.sourceConfigPath ??
      path.join(repoRoot, 'packages/wallet-console-server-ts/wrangler.d1-local.toml'),
  );
  ...
  const localConsoleOrganizationId = resolveLocalConsoleOrganizationId({
    localEnvRoot,
    organizationId: input.localConsoleOrganizationId,
  });
  ...
  runtimeConfig = replaceTomlAssignment(
    ...
    requiredScope(input.localConsoleProjectId, 'SEAMS_LOCAL_CONSOLE_PROJECT_ID'),
  );
  runtimeConfig = replaceTomlAssignment(
    runtimeConfig,
...
function requiredScope(value, name) {
  const normalized = String(value || '').trim();
  if (!normalized)
    throw new Error(`Configure ${name} with the workspace created through the Console`);
  return normalized;
}
