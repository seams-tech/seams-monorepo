...
const UI_STATE_STORAGE_KEY = 'seams-dashboard-ui-state-v1';
...
type PersistedUiState<GroupKey extends string = string> = {
...
type UseDashboardUiPreferencesInput<GroupKey extends string> = {
  dropdownOptions: TopbarDropdownOptions;
  defaultContext: TopbarContextState;
  groupKeys: readonly GroupKey[];
  defaultExpandedGroups: ExpandedSidebarGroupsState<GroupKey>;
};
...
function sanitizeExpandedGroups<GroupKey extends string>(
  input: Partial<ExpandedSidebarGroupsState<GroupKey>> | undefined,
  groupKeys: readonly GroupKey[],
  defaultExpandedGroups: ExpandedSidebarGroupsState<GroupKey>,
): ExpandedSidebarGroupsState<GroupKey> {
  const resolved = {} as ExpandedSidebarGroupsState<GroupKey>;
  for (const key of groupKeys) {
    resolved[key] = input?.[key] ?? defaultExpandedGroups[key];
  }
  return resolved;
}
...
function readRawStoredState<GroupKey extends string = string>(): Partial<
  PersistedUiState<GroupKey>
> {
  ...
  try {
    ...
    return parsed as Partial<PersistedUiState<GroupKey>>;
  } catch {
    return {};
  }
}

function readStoredState<GroupKey extends string>(
  dropdownOptions: TopbarDropdownOptions,
  defaultContext: TopbarContextState,
  groupKeys: readonly GroupKey[],
  defaultExpandedGroups: ExpandedSidebarGroupsState<GroupKey>,
): Partial<PersistedUiState<GroupKey>> {
  const parsed = readRawStoredState<GroupKey>();
  ...
  return {
    ...
    expandedGroups: sanitizeExpandedGroups(parsed.expandedGroups, groupKeys, defaultExpandedGroups),
...
function parseExpandedGroupsParam<GroupKey extends string>(
  raw: string | null,
  groupKeys: readonly GroupKey[],
): Partial<ExpandedSidebarGroupsState<GroupKey>> | undefined {
  ...
  const result: Partial<ExpandedSidebarGroupsState<GroupKey>> = {};
  for (const key of groupKeys) {
...
function readUrlState<GroupKey extends string>(
  dropdownOptions: TopbarDropdownOptions,
  defaultContext: TopbarContextState,
  groupKeys: readonly GroupKey[],
  defaultExpandedGroups: ExpandedSidebarGroupsState<GroupKey>,
    ...
    ...(hasExpandedGroupOverride
      ? {
          expandedGroups: sanitizeExpandedGroups(
            parseExpandedGroupsParam(expandedGroupsRaw, groupKeys),
            groupKeys,
...
function readInitialState<GroupKey extends string>(
  dropdownOptions: TopbarDropdownOptions,
  defaultContext: TopbarContextState,
  groupKeys: readonly GroupKey[],
  defaultExpandedGroups: ExpandedSidebarGroupsState<GroupKey>,
): PersistedUiState<GroupKey> {
  const baseState: PersistedUiState<GroupKey> = {
    isSidebarExpanded: true,
    expandedGroups: { ...defaultExpandedGroups },
    selectedContext: sanitizeSelectedContext(undefined, dropdownOptions, defaultContext),
  };
  const stored = readStoredState(dropdownOptions, defaultContext, groupKeys, defaultExpandedGroups);
  const fromUrl = readUrlState(dropdownOptions, defaultContext, groupKeys, defaultExpandedGroups);
...
type UseDashboardUiPreferencesResult<GroupKey extends string> = {
  isSidebarExpanded: boolean;
  expandedGroups: ExpandedSidebarGroupsState<GroupKey>;
  selectedContext: TopbarContextState;
  toggleSidebar: () => void;
...
export function useDashboardUiPreferences<GroupKey extends string>(
  pathname: string,
  input: UseDashboardUiPreferencesInput<GroupKey>,
): UseDashboardUiPreferencesResult<GroupKey> {
  const { dropdownOptions, defaultContext, groupKeys, defaultExpandedGroups } = input;
  const initialState = React.useMemo(
    () => readInitialState(dropdownOptions, defaultContext, groupKeys, defaultExpandedGroups),
    [defaultContext, defaultExpandedGroups, dropdownOptions, groupKeys],
  );

  const [isSidebarExpanded, setIsSidebarExpanded] = React.useState<boolean>(
    initialState.isSidebarExpanded,
  );
  const [expandedGroups, setExpandedGroups] = React.useState<ExpandedSidebarGroupsState<GroupKey>>(
  ...
  const toggleGroup = React.useCallback((group: GroupKey) => {
