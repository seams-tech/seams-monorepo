...
const strictWorkerBuildRoot = join(walletServerRoot, 'cloudflare-router-ab', 'build');
let strictRuntime;
let d1Runtime;
...
function assertProductionWorkerBinariesReady() {
  const missing = missingProductionWorkerArtifactPaths();
  if (missing.length > 0) {
    throw new Error('The installed Wallet release is missing Worker artifacts: ' + missing.join(', '));
  }
}
