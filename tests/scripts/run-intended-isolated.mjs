...
function runCase(testCase, index, total) {
  ...
  const environment = {
    ...
    SEAMS_INTENDED_D1_PERSIST_TO: path.join(caseRoot, '.local', 'cloudflare-state', 'gateway'),
    SEAMS_INTENDED_D1_WRANGLER_RUNTIME_DIR: path.join(caseRoot, '.runtime', 'wrangler-d1-local'),
    SEAMS_D1_LOCAL_WRANGLER_CONFIG: path.join(
      caseRoot,
      '.runtime',
      'wrangler-d1-local',
      'wrangler.d1-local.toml',
    ),
    SEAMS_INTENDED_TEST_APP_VITE_CACHE_DIR: viteCacheDir,
