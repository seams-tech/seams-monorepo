...
test.describe('D1 registration ECDSA wallet keys', () => {
  ...
  test('rejects malformed or extended ECDSA public identities at the D1 record boundary', () => {
    const bootstrap = makeBootstrap({ targetLabel: 'parser' });

    expect(
      parseD1EcdsaDerivationServerBootstrapResponse({
        ...bootstrap,
        publicIdentity: {
          ...bootstrap.publicIdentity,
          derivationClientSharePublicKey33B64u: 'malformed-public-key',
        },
      }),
    ).toBeNull();
