...
export type TenantRootRefreshResumptionRunner = (input: {
  readonly scheduledTimeMs: number | undefined;
...
export interface CloudflareCronOptions {
  ...
  /**
   * Optional bounded replay of durable tenant-root refresh operations.
   * The runner owns tenant scoping and admission; this dispatcher only gates
   * it to the configured cron tick.
   */
  tenantRootRefreshResumption?: {
    readonly cronExpressions?: string[];
    readonly runner: TenantRootRefreshResumptionRunner;
  };
}
...
export function createCloudflareCron(opts: CloudflareCronOptions = {}): ScheduledHandler {
  ...
  const tenantRootRefreshResumption = opts.tenantRootRefreshResumption;
  const tenantRootRefreshResumptionCronExpressions = normalizeCronExpressions(
    tenantRootRefreshResumption?.cronExpressions,
  );
  return async (event) => {
    if (verbose) {
      logger.info('[cron] tick', {
        ...
        tenantRootRefreshResumption: Boolean(tenantRootRefreshResumption),
      });
    }
    ...
    if (tenantRootRefreshResumption) {
      const eventCron = typeof event?.cron === 'string' ? event.cron : undefined;
      if (!shouldRunForCronTick(tenantRootRefreshResumptionCronExpressions, eventCron)) {
        if (verbose) {
          logger.info('[cron][tenant-root-refresh-resumption] skipped: cron expression mismatch', {
            eventCron,
            cronExpressions: tenantRootRefreshResumptionCronExpressions,
          });
        }
      } else {
        try {
          await tenantRootRefreshResumption.runner({
            scheduledTimeMs:
              typeof event?.scheduledTime === 'number' ? event.scheduledTime : undefined,
            cron: eventCron,
          });
        } catch (error: unknown) {
          logger.warn('[cron][tenant-root-refresh-resumption] failed', {
