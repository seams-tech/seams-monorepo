...
Prepare Wallet-system and Console generations independently:

```bash
pnpm wallet-system:deploy:env-prepare -- --lane staging-testnet --repo seams-tech/seams-monorepo
pnpm console:deploy:env-prepare -- --lane staging-testnet
```

The Wallet-system command generates Router A/B identities, the temporary
shared internal-service credential, Gateway random secrets, ceremony JWT key,
and signing-session seal values. The Console command separately generates its
session, invitation, webhook, and grant-authority values. Each writes its own
protected manifest and generation metadata. Supply provisioned Cloudflare,
domain, funded-account, OAuth, billing, and tenant values through the protected
authority-specific files documented in
[tooling.md](tooling.md#deployment-authorities). The output
contains private material and must not be committed.

Apply each saved authority manifest explicitly:

```bash
gh auth login
pnpm wallet-system:deploy:env-apply -- \
  --lane staging-testnet --manifest-file <wallet-system-manifest> --rotate
pnpm console:deploy:env-apply -- \
  --lane staging-testnet --manifest-file <console-manifest>
```

Prepare mode resolves external values from the protected file and current shell,
discovers Wallet-system Cloudflare resources when required, and refuses a
partial required configuration. Apply preserves existing GitHub environments
and their protection rules. Every preparation creates a new generation. Use
`--rotate` only for an intentional Wallet identity rotation.
...
recorded in `deployment/wallet-system/targets.json`; never reuse production-mainnet private
material for production-testnet.

Progress and per-environment upload counts are written to stderr. Restricted
Wallet-system backups are stored under `$HOME/.seams/backups`; Console backups
are stored under `$HOME/.seams/backups/console`. Move them to the approved
secrets vault according to the retention policy.
...
`--repo seams-tech/seams-monorepo`. Do not copy placeholder text such as
`owner/repo`.
...
`deployment/wallet-system/targets.json` owns the non-secret Wallet-system
configuration: signer D1 resource IDs, runtime tenant identity, origins, Router A/B public
identity, session settings, and optional integration configuration. The
Console Worker/D1/origin and billing targets live separately in
`deployment/console/targets.json`. The deployment target parser validates both
documents and composes their non-secret service handoff for existing renderers.
Tenant identifiers are configuration only; deployment creates no tenant rows.
...
secrets. Public keys and Wallet deployment metadata are reviewed in
`deployment/wallet-system/targets.json`; Console resource changes are reviewed
in `deployment/console/targets.json`.
...
```bash
pnpm --dir packages/wallet-console-server-ts run d1:local:restore:drill
```
...
Use `wallet-system:deploy:env-prepare` and `wallet-system:deploy:env-apply` to generate
...
The Wallet-system backend workflow applies signer D1 migrations before the
ordered Worker deployments: SigningWorker, Deriver A, Deriver B, MPCRouter,
Wallet Runtime, and Gateway. Each job performs its component preflight from
explicit environment inputs. The separate Console workflow applies Console D1
migrations and deploys the Console Worker from its Console environment.
Backend smoke runs at the end of the Gateway job.
...
```bash
pnpm --dir packages/wallet-console-server-ts run d1:local:prepare
pnpm --dir packages/wallet-console-server-ts run d1:local:dev
```

The local config is
`packages/wallet-console-server-ts/wrangler.d1-local.toml`. It binds `seams-console` to
...
cp packages/wallet-console-server-ts/wrangler.d1-staging-console.toml.example \
  packages/wallet-console-server-ts/wrangler.d1-staging-console.toml
cp packages/wallet-console-server-ts/wrangler.d1-staging-gateway.toml.example \
  packages/wallet-console-server-ts/wrangler.d1-staging-gateway.toml
pnpm --dir packages/wallet-console-server-ts run d1:staging:check
...
```bash
pnpm --dir packages/wallet-console-server-ts run d1:staging:runbook -- \
...
```bash
pnpm --dir packages/wallet-console-server-ts run d1:staging:resources -- --mode dry-run
pnpm --dir packages/wallet-console-server-ts run d1:staging:resources -- --mode remote
```
...
```bash
pnpm --dir packages/wallet-console-server-ts run d1:staging:bookmark -- \
  --mode remote \
...
```bash
pnpm --dir packages/wallet-console-server-ts run d1:staging:import-fixtures -- \
  --mode dry-run \
...
```bash
pnpm --dir packages/wallet-console-server-ts run d1:staging:smoke -- \
  --mode remote \
  --console-origin <console-staging-origin> \
  --gateway-origin <gateway-staging-origin>
```
...
```bash
pnpm --dir packages/wallet-console-server-ts run d1:staging:reconcile -- --mode dry-run
pnpm --dir packages/wallet-console-server-ts run d1:staging:reconcile -- --mode remote
```
...
pnpm --dir packages/wallet-console-server-ts run d1:staging:signer-custody -- \
  --mode dry-run \
  --gateway-origin <gateway-staging-origin> \
...
`packages/wallet-console-server-ts/.wrangler/d1-staging-signer-custody` and never records
...
```bash
pnpm --dir packages/wallet-console-server-ts run d1:staging:r2-restore-drill -- \
  --mode dry-run \
  --r2-bucket <staging-r2-backup-bucket>
pnpm --dir packages/wallet-console-server-ts run d1:staging:r2-restore-drill -- \
  --mode remote \
...
```bash
pnpm --dir packages/wallet-console-server-ts run d1:staging:evidence -- \
