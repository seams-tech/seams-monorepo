...
class AvailableSigningSessionSealsRepository extends SigningSessionSealsRepository {
  putCalls = 0;
...
test.describe('R102 holder delivery worker boundary', () => {
  test('rejects a changed holder record for an existing exact store key', async () => {
    const job = buildR102LaneJob('holder-record-conflict');
    const record = buildR103SealedHolderRecord(job, buildR102ProtocolCommitReceipt(job));
    const conflicting = {
      ...record,
      sealedHolderMaterialB64u: Buffer.from([2]).toString('base64url'),
    };
    const seals = new AvailableSigningSessionSealsRepository();
    const repository = new LaneSealedHolderMaterialRepository(seals);

    await repository.put(record);
    await expect(repository.put(conflicting)).rejects.toThrow(
      'lane sealed holder record conflicts with its exact store key',
    );
    expect(seals.putCalls).toBe(1);
  });
