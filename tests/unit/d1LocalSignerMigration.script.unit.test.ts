...
const modulePath = path.join(
  root,
  'packages/wallet-server/scripts/d1-local-migrate-signer.mjs',
);
const migrationPath = path.join(
  root,
  'packages/wallet-server/migrations/d1-signer/0028_r103f_phase1_additive_schema_bridge.sql',
);
const configPath = path.join(root, 'packages/wallet-console-server-ts/wrangler.d1-local.toml');
...
test('local signer compatibility config redirects only signer migrations', async () => {
  ...
  expect(compatible).toContain('migrations_dir = "migrations/d1-console"');
});
