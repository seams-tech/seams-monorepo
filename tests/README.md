...
`pnpm test:console` is the local and CI entrypoint. The five journeys in
`e2e/console/*.operating.test.ts` own route/data smoke, new-owner setup, policy
governance, webhook failure/recovery, and billing funding/documents. Extend
these journeys for related behavior; a separate journey needs a distinct
user-facing outcome.

`playwright.console.config.ts` runs Chromium with one worker, zero retries,
and a fresh managed stack from `scripts/start-intended-services.mjs`. It retains
failure traces, screenshots, and video. The repository validation workflow
retains failure artifacts for seven days. Service logs remain in the CI log.

Use the real Console frontend, Caddy routing, local worker, and fresh D1
state. Local authentication uses the existing `X-Console-*` boundary through
`e2e/console/harness.ts`. Onboard prerequisites through public APIs, give each
journey a unique tenant, and keep raw D1 writes and fabricated session records
out of test bodies. Billing uses the existing synthetic provider; webhooks
reach a test-owned HTTP receiver. External identity/provider services are
covered separately at their boundaries.

Each mutation needs durable or external evidence: reload a D1-backed resource,
read through another page/API, inspect the published snapshot or audit trail,
receive the signed webhook, or reconcile ledger and billing-document views.
Use accessible roles and stable domain identifiers. Exact incidental copy,
CSS classes, request counts, and complete response snapshots do not establish
an operating outcome.

Focused lower suites retain D1 isolation/atomicity, RBAC and malformed-input
rejection, provider signatures, ledger accounting, webhook HMAC/retry claims,
and encoding invariants. Remove a lower test only when an operating journey
owns the same outcome and no distinct boundary invariant is lost. A green
browser suite does not replace those checks. Fix failures before adding retries.
