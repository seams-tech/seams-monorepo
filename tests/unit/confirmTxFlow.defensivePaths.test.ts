...
test.describe('confirmTxFlow – defensive paths', () => {
  ...
  test('AUTHORIZE_KEY_EXPORT uses only the exact credential selected by the export lane', async ({
    page,
  }) => {
    const result = await page.evaluate(
      async ({ paths }) => {
  ...
  test('AUTHORIZE_KEY_EXPORT forwards a caller-bound challenge when provided', async ({
    page,
  }) => {
    const result = await page.evaluate(
      async ({ paths }) => {
