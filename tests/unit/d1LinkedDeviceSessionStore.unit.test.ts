...
test('matches QR identity independent of field order and rejects changed permission', async () => {
  temporary = await openDatabase();
  const fixture = buildR103DeviceLinkFixture({ linkSessionId: 'link-session:r103-qr-equality' });
  const service = buildService(fixture);
  const reorderedPayload: typeof fixture.payload = {
    targetFactor: fixture.payload.targetFactor,
    expiresAtMs: fixture.payload.expiresAtMs,
    issuedAtMs: fixture.payload.issuedAtMs,
    requestedPermission: fixture.payload.requestedPermission,
  ...
  const changedPayload: typeof fixture.payload = {
    ...reorderedPayload,
    requestedPermission: buildFullOwnerDelegatedWalletAuthorityV1(),
  };

  expect(linkedDeviceQrPayloadsEqualV1(fixture.payload, reorderedPayload)).toBe(true);
  expect(linkedDeviceQrPayloadsEqualV1(fixture.payload, changedPayload)).toBe(false);

  await expect(
    service.createUnclaimedSessionV1({ payload: fixture.payload, nowMs }),
  ).resolves.toMatchObject({ outcome: 'applied' });
  await expect(
    service.createUnclaimedSessionV1({ payload: reorderedPayload, nowMs }),
  ).resolves.toMatchObject({ outcome: 'replayed' });
  await expect(
    service.createUnclaimedSessionV1({ payload: changedPayload, nowMs }),
