...
test.describe('NearClient structured RPC failure classification', () => {
  test('classifies transaction, account, and access-key absence from structured causes', () => {
    const transaction = NearRpcError.fromRpcError('Tx Status', {
      name: 'HANDLER_ERROR',
    ...
    const account = NearRpcError.fromRpcError('View Account', {
      name: 'HANDLER_ERROR',
      message: 'Server error',
      cause: { name: 'UNKNOWN_ACCOUNT', info: { account_id: 'missing.testnet' } },
    });
    const accessKey = NearRpcError.fromRpcError('View Access Key', {
      name: 'HANDLER_ERROR',
      message: 'Server error',
      data: {
  ...
  test('does not infer failure kind from English message text', () => {
    const error = NearRpcError.fromRpcError('Tx Status', {
      name: 'HANDLER_ERROR',
  ...
  test('classifies InvalidNonce and Expired from structured execution failures', () => {
    ...
    const expired = NearRpcError.fromRpcError('Send Transaction', {
      message: 'Server error',
      data: {
        TxExecutionError: {
