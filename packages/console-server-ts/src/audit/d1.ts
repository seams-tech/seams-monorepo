...
export const CONSOLE_AUDIT_CATEGORIES = [
  'POLICY',
  'SETTINGS',
  'KEY_EXPORT',
  'BILLING',
...
export const CONSOLE_AUDIT_EVIDENCE_REFERENCE_KINDS = [
  'LOG',
  'EXPORT',
  'PAYMENT',
...
interface D1ConsoleAuditState {
  ...
  readonly now: () => Date;
  readonly categories: ReadonlySet<ConsoleAuditCategory>;
  readonly evidenceReferenceKinds: ReadonlySet<ConsoleAuditEvidenceReferenceKind>;
}
...
export interface D1ConsoleAuditServiceOptions {
  ...
  readonly now?: () => Date;
  readonly additionalCategories?: readonly ConsoleAuditCategory[];
  readonly additionalEvidenceReferenceKinds?: readonly ConsoleAuditEvidenceReferenceKind[];
}
...
export async function createD1ConsoleAuditService(
  ...
  const state: D1ConsoleAuditState = {
    ...
    categories: new Set([...CONSOLE_AUDIT_CATEGORIES, ...(options.additionalCategories || [])]),
    evidenceReferenceKinds: new Set([
      ...CONSOLE_AUDIT_EVIDENCE_REFERENCE_KINDS,
      ...(options.additionalEvidenceReferenceKinds || []),
    ]),
  };
...
function parseCategory(
  raw: unknown,
  categories: ReadonlySet<ConsoleAuditCategory>,
): ConsoleAuditCategory {
  const value = normalizeString(raw).toUpperCase() as ConsoleAuditCategory;
  return categories.has(value) ? value : 'SYSTEM';
}
...
function parseReference(
  raw: unknown,
  evidenceReferenceKinds: ReadonlySet<ConsoleAuditEvidenceReferenceKind>,
): ConsoleAuditEvidenceReference | null {
  if (!raw || typeof raw !== 'object' || Array.isArray(raw)) return null;
  const row = raw as Record<string, unknown>;
  const kind = normalizeString(row.kind).toUpperCase() as ConsoleAuditEvidenceReferenceKind;
  if (!evidenceReferenceKinds.has(kind)) return null;
...
function parseReferences(
  ...
  for (const entry of parseJsonArray(raw)) {
    const parsed = parseReference(entry, evidenceReferenceKinds);
...
function parseEventRow(
  ...
  return {
    ...
    category: parseCategory(row.category, categories),
...
function parseEvidenceRow(
  ...
  return {
    ...
    references: parseReferences(row.references_json, evidenceReferenceKinds),
    createdAt: toIso(toNumber(row.created_at_ms)),
  };
}
...
function ensureReferences(
  ...
  for (const entryRaw of raw) {
    const entry = parseReference(entryRaw, evidenceReferenceKinds);
    if (!entry) continue;
...
class D1ConsoleAuditServiceImpl implements ConsoleAuditService {
  ...
  async listEvents(
    ...
    const out = await this.state.database
      ...
      .all<D1Row>();
    return (out.results || []).map((row) => parseEventRow(row, this.state.categories));
  }
  ...
  async listEvidence(
    ...
    return (out.results || []).map((row) =>
      parseEvidenceRow(row, this.state.evidenceReferenceKinds),
    );
  }
  ...
  private buildEvent(input: {
    ...
    return {
      ...
      category: ensureCategory(input.request.category, this.state.categories),
  ...
  private buildEvidence(input: {
    ...
    return {
      ...
      references: ensureReferences(input.request.references, this.state.evidenceReferenceKinds),
      createdAt: toIso(input.createdAtMs),
    };
  }
}
