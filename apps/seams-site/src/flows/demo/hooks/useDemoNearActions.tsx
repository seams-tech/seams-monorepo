...
export function useDemoNearActions(args: UseDemoNearActionsArgs) {
  ...
  const handleSignDelegateGreeting = useCallback(async () => {
    ...
    try {
      ...
      // the greeting silently never updates. viewAccessKey returns both the
      // sender key's current nonce and the block height it was resolved against.
      const nearClient = new MinimalNearClient(FRONTEND_CONFIG.nearRpcUrl);
      const accessKey = await nearClient.viewAccessKey(nearAccountId!, delegatePublicKey);
      ...
      const result = await seams.near.signDelegateAction({
        ...
        delegate: {
          ...
          nonce: (BigInt(accessKey.nonce) + 1n).toString(),
          maxBlockHeight: accessKey.block_height + DELEGATE_MAX_BLOCK_HEIGHT_BUFFER,
