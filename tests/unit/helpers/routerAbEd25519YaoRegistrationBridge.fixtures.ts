...
function retainSideEffectRecord<T, P>(
  record: RouterAbEd25519YaoRegistrationSideEffectRecordV1<T, P>,
  ...
  switch (record.kind) {
    case 'router_ab_ed25519_yao_registration_side_effect_claim_v1':
      return {
        ...
        preparedArtifactFingerprint: record.preparedArtifactFingerprint,
...
> implements RouterAbEd25519YaoRegistrationSideEffectStoreV1<T, P> {
  ...
  > {
    ...
    return record
      ? {
          kind: 'present',
          value: retainSideEffectRecord(record.value),
  ...
  async put(
    ...
    if (
      ...
      this.records.set(key, {
        ...
        value: retainSideEffectRecord(this.claimWinner),
      });
    ...
    if (
      ...
      this.records.set(key, {
        version: (current?.version ?? 0) + 1,
        value: retainSideEffectRecord(this.terminalWinner),
      });
      this.terminalWinner = null;
      return { kind: 'version_mismatch' };
    }
    const version = (current?.version ?? 0) + 1;
    this.records.set(key, { version, value: retainSideEffectRecord(value) });
