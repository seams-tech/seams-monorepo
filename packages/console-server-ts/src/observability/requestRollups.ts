...
export function shouldCaptureConsoleObservabilityRequestMetric(input: {
  ...
  method: string;
  statusCode: number;
  policies?: readonly ConsoleObservabilityRequestMetricPolicy[];
}): boolean {
  ...
  return resolveConsoleObservabilityRequestMetricPolicy(input.routeFamily, input.policies) !== null;
}
...
export function normalizeConsoleObservabilityRequestMetricForInsert(
  ...
  const policy = resolveConsoleObservabilityRequestMetricPolicy(routeFamily, policies);
  if (
    !policy ||
