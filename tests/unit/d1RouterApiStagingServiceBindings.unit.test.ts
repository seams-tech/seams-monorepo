...
} from '../../packages/wallet-server/src/router/cloudflare/runtime/routerAbServiceBindings';
...
class UnexpectedPrewarmResponseBinding extends RecordingServiceBinding {
...
test('Gateway one-minute schedule calls the authenticated MPC Router prewarm path', async () => {
  const bindings = createBindings();
  await runRouterAbPrewarmScheduledV1(
    { cron: '* * * * *' },
    {
...
test('Gateway rejects a prewarm response with unsupported fields', async () => {
  const binding = new UnexpectedPrewarmResponseBinding();
  ...
  ).rejects.toThrow('Router A/B prewarm returned an invalid response');
