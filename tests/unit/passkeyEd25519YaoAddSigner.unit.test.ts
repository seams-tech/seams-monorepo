...
async function addSignerInput(): Promise<VerifiedPasskeyEd25519YaoAddSignerPreparationInputV1> {
...
type AdmissionMutation = {
  ...
  mutate(input: VerifiedPasskeyEd25519YaoAddSignerPreparationInputV1): void;
};
...
test.describe('verified passkey Ed25519 Yao add-signer authorization', () => {
  test('builds the exact admission and HTTP transport from verified add-signer facts', async () => {
    const input = await addSignerInput();
    ...
    expect(prepared).toEqual({
      ...
      transportConfig: {
        routerOrigin: 'http://127.0.0.1:8787',
        authorization: { kind: 'bearer', value: 'Bearer add-signer-yao-grant' },
        fetch: UNUSED_FETCH,
      },
    });
  });
  ...
  test('rejects authority substitution before Yao execution', async () => {
    const input = await addSignerInput();
  ...
  test('rejects invalid transport configuration before Yao execution', async () => {
    const input = await addSignerInput();
    input.httpTransport.routerOrigin = 'file:///tmp/router';

    await expect(admitVerifiedPasskeyEd25519YaoAddSignerV1(input)).rejects.toThrow(
