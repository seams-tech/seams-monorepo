...
test('persists strict encrypted pending records for Passkey and Google Email OTP', async () => {
  ...
  const passkeyRow = await toPendingWalletRecoveryCommitAppStateRow(passkeyPending);
  if (passkeyRow.value.record.stage !== 'server_promoted' || !passkeyRow.value.record.projection) {
    throw new Error('passkey recovery fixture did not produce a promoted projection');
  }
  const projection = passkeyRow.value.record.projection;
  const reorderedAuthorityRow = {
    ...passkeyRow,
    value: {
      ...passkeyRow.value,
      record: {
        ...passkeyRow.value.record,
        projection: {
          ...projection,
          authority: {
  ...
  expect(pendingWalletRecoveryCommitAppStateRowsMatch(reorderedAuthorityRow, passkeyRow)).toBe(
    true,
  );

  const changedAuthorityRow = {
    ...reorderedAuthorityRow,
    value: {
      ...reorderedAuthorityRow.value,
      record: {
        ...reorderedAuthorityRow.value.record,
        projection: {
          ...projection,
          authority: {
            ...projection.authority,
            updatedAtMs: projection.authority.updatedAtMs + 1,
          },
        },
      },
    },
  };
  expect(pendingWalletRecoveryCommitAppStateRowsMatch(changedAuthorityRow, passkeyRow)).toBe(false);
});
...
test('deletes a definite server refusal from either pending recovery stage', async ({ page }) => {
  ...
  const result = await page.evaluate(
    async ({ paths, payload, projection }) => {
      ...
      const localMaterial = await encryptWalletRecoveryDurablePayload(
        parseWalletRecoveryDurablePayload(payload),
      );
      const awaiting = await buildPendingWalletRecoveryCommitV1({
        kind: 'pending_wallet_recovery_commit_v1',
        version: 1,
