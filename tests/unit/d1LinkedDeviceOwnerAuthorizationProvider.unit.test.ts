...
import { linkedDeviceSignerManifestsEqualV1 } from '../../packages/wallet-server/src/router/cloudflare/d1/deviceLinking/d1LinkedDeviceVerifiedLinkBuilder';
...
test('compares linked-device signer manifests by ordered domain fields', () => {
  const manifest = buildR103DeviceLinkFixture().sourceSignerManifest;
  const insertionOrderVariant = parseExactAdministeredSignerManifestV1({
    signers: manifest.signers,
    keyFamilies: manifest.keyFamilies,
    kind: manifest.kind,
  });
  expect(linkedDeviceSignerManifestsEqualV1(manifest, insertionOrderVariant)).toBe(true);

  const signer = manifest.signers[0];
  if (!signer || signer.keyFamily !== 'ed25519') {
    throw new Error('expected the manifest fixture to contain an Ed25519 signer');
  }
  const changed = buildExactAdministeredSignerManifestV1([
    {
      ...signer,
  ...
  expect(linkedDeviceSignerManifestsEqualV1(manifest, changed)).toBe(false);
});
