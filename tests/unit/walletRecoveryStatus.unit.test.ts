...
import { readWalletRecoveryCodeStatus } from '../../packages/wallet/src/core/rpcClients/relayer/walletRecoveryRotate';
...
test('the wallet client accepts only the exact recovery status response', async () => {
  const exact = await readWalletRecoveryCodeStatus({
  ...
  const extraField = await readWalletRecoveryCodeStatus({
    relayUrl: 'https://relay.localhost',
    walletId: 'alice.testnet',
    fetchImpl: async () =>
      new Response(
        JSON.stringify({
          ...
          storeVersion: '4',
          backupOutstanding: true,
          recoveryKeyIds: ['must-not-cross-the-boundary'],
        }),
        { status: 200 },
      ),
  });
  expect(extraField).toEqual({
    ...
    message: 'recovery status returned an unusable payload',
  });
});
