...
test.describe('threshold ECDSA key identity inventory parser', () => {
  ...
  test('rejects an inventory capability for another wallet or threshold public key', () => {
    const canonicalCapability = publicCapability();
    const wrongWalletCapability = {
      ...canonicalCapability,
      client_id: 'other.testnet',
    };
    const wrongPublicKeyCapability = {
      ...canonicalCapability,
      public_identity: {
        ...canonicalCapability.public_identity,
        threshold_public_key33_b64u: Buffer.from([
          2,
