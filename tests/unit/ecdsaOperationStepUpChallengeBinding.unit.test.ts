...
import {
  ...
  parseRouterAbEcdsaExplicitExportRequestFactsV1,
...
test.describe('ECDSA operation step-up challenge binding', () => {
  ...
  test('client worker export request parser admits exact facts only', () => {
    const request = parseRouterAbEcdsaDerivationExplicitExportRequestV1(
      explicitExportRequest('evm.export_key'),
    );
    ...
    const facts = {
      ...factsWithoutEnvelopes,
      authorization: { kind: 'operation_step_up' as const },
      operation: request.operation,
    ...
    const parsedFacts = parseRouterAbEcdsaExplicitExportRequestFactsV1(facts);
    expect(parsedFacts.authorization.kind).toBe('operation_step_up');
    expect(parsedFacts.operation?.operation_kind).toBe('evm.export_key');

    const parsedOuter = parseCreateRouterAbEcdsaPostRegistrationCeremonyRequestV1({
      kind: 'create_router_ab_ecdsa_explicit_export_ceremony_v1',
      ceremonyId: 'ceremony-1',
      request: facts,
    });
    expect(parsedOuter.ceremonyId).toBe('ceremony-1');
    expect(() =>
      parseRouterAbEcdsaExplicitExportRequestFactsV1({ ...facts, unexpected: true }),
  ...
  test('SigningWorker export binding preserves the verified step-up protocol label', () => {
    ...
    expect(parseRouterAbEcdsaSigningWorkerExportShareBindingV1(binding)).toEqual(binding);
    expect(() =>
      parseRouterAbEcdsaSigningWorkerExportShareBindingV1({ ...binding, unexpected: true }),
    ).toThrow(/unexpected is not a supported field/);
    const { key_handle: _keyHandle, ...withoutKeyHandle } = binding;
    expect(() => parseRouterAbEcdsaSigningWorkerExportShareBindingV1(withoutKeyHandle)).toThrow(
      /key_handle must be a string/,
    );
    expect(() =>
      parseRouterAbEcdsaSigningWorkerExportShareBindingV1({
        ...binding,
    ...
    expect(() => parseRouterAbEcdsaExplicitExportProtocolForwardedResponseV1(responseBody)).toThrow(
      /material_activation\.activation_id is not a supported field/,
    );
    expect(
      parseRouterAbEcdsaExplicitExportProtocolForwardedResponseV1({
        ...responseBody,
      ...
      }).signing_worker_export.binding.material_activation,
    ).toEqual(normalSigningScope.material_activation);
    expect(() =>
      parseRouterAbEcdsaExplicitExportForwardedResponseV1({
        ...responseBody,
        signing_worker_export: {
