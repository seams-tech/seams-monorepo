...
import {
  ...
  upgradeWalletCustodyEnvelopeOwnership,
...
test('an active envelope carries its store version', async () => {
  ...
  expect(result).toEqual({ kind: 'active', envelope, storeVersion: 'v7' });
});
...
test('a 200 missing its store version is corrupt, not active', async () => {
  ...
  // the cause.
  const result = await fetchWith(200, { ok: true, envelope: passkeyCustodyEnvelope() });
  expect(result.kind).toBe('corrupt');
});

test('a 200 with an extended envelope response is corrupt, not active', async () => {
  const result = await fetchWith(200, {
    ok: true,
...
test('ownership upgrade rejects an extended success response', async () => {
  const result = await upgradeWalletCustodyEnvelopeOwnership({
    relayUrl: 'https://relay.localhost',
    walletId: 'alice.testnet',
    walletSessionToken: 'session-token',
    envelope: passkeyCustodyEnvelope(),
    fetchImpl: respondWith(200, { ok: true, upgraded: false, legacy: true }),
  });
