...
const aliasConfig = {
  ...
  '@seams-internal/wallet-console-shared': path.resolve(
    PACKAGE_ROOT_ABS,
    '../wallet-console-shared-ts/src/index.ts',
  ),
  '@seams-internal/wallet-console-shared/*': path.resolve(
    PACKAGE_ROOT_ABS,
    '../wallet-console-shared-ts/src/*',
  ),
  '@seams-internal/wallet-console-server': path.resolve(PACKAGE_ROOT_ABS, 'src/index.ts'),
  '@seams-internal/wallet-console-server/*': path.resolve(PACKAGE_ROOT_ABS, 'src/*'),
};

const preservedEntryFileNames = (chunk: { facadeModuleId?: string | null; name: string }) => {
  if (chunk.facadeModuleId?.includes('\0')) return '_virtual/[name]-[hash].js';
