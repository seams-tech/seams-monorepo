...
async function deliverEventToEndpoints(
  ctx: ConsoleWebhooksContext,
  request: Required<EmitConsoleWebhookEventRequest>,
  targets: StoredWebhookEndpoint[],
  now: () => Date,
  getDeliveries: (endpointId: string) => StoredWebhookDelivery[],
  dispatch: (
  ...
  const { eventId, eventType } = request;
  let delivered = 0;
  let failed = 0;

  for (const endpoint of targets) {
    const existingDelivery = getDeliveries(endpoint.id).find((entry) => entry.eventId === eventId);
    if (existingDelivery) {
    ...
    getDeliveries(endpoint.id).push(delivery);
    await dispatch(endpoint, delivery, false);
...
export function createInMemoryConsoleWebhookService(
  ...
  return {
    ...
    async sendTestEvent(
      ...
      if (!endpoint)
        throw new ConsoleWebhookError('webhook_not_found', 404, 'Webhook endpoint was not found');
      if (endpoint.status !== 'ACTIVE') {
        throw new ConsoleWebhookError(
          'endpoint_disabled',
          409,
          'Enable the endpoint before sending a test event.',
        );
      }
      return deliverEventToEndpoints(
        ctx,
        createWebhookTestEvent(endpointId, now()),
        [endpoint],
        now,
        getEndpointDeliveries.bind(null, store),
        deliver.bind(null, store, ctx),
      );
    },
    ...
    async emitEvent(
      ...
      return deliverEventToEndpoints(
        ctx,
        { eventId, eventType, payload: request.payload },
        targets,
        now,
        getEndpointDeliveries.bind(null, store),
        deliver.bind(null, store, ctx),
