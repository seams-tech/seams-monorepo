...
function expectCredentialFreeRegistrationJournal(
  ...
  const completion = Object.fromEntries(Object.entries(raw));
  const rawReceipt = completion.receipt;
  if (rawReceipt === null || typeof rawReceipt !== 'object' || Array.isArray(rawReceipt)) {
    throw new Error('registration journal row is not a completion receipt');
  }
  const receipt = Object.fromEntries(Object.entries(rawReceipt));
  expect(completion.kind).toBe('router_ab_ed25519_yao_registration_side_effect_completion_v2');
  expect(completion.operation).toBe(operation);
  expect(receipt.kind).toBe('wallet_registration_session_commit_receipt_v2');
...
function clonedRecord(value: unknown): Record<string, unknown> {
  ...
  return Object.fromEntries(Object.entries(clone));
}
...
test('identical activate retries return one credential-free committed projection', async () => {
  ...
  try {
    ...
    const completionRecord = Object.fromEntries(Object.entries(completion));
    const rawReceipt = completionRecord.receipt;
    if (rawReceipt === null || typeof rawReceipt !== 'object' || Array.isArray(rawReceipt)) {
      throw new Error('activation completion receipt is missing');
    }
    const receipt = Object.fromEntries(Object.entries(rawReceipt));
    const requestFingerprint = completionRecord.requestFingerprint;
    const rawCommitted = receipt.committed;
    if (
      typeof requestFingerprint !== 'string' ||
      rawCommitted === null ||
      typeof rawCommitted !== 'object' ||
      Array.isArray(rawCommitted)
    ) {
      throw new Error('activation completion identity is missing');
    }
    const committed = Object.fromEntries(Object.entries(rawCommitted));
    const rawSession = committed.session;
    ...
    const session = Object.fromEntries(Object.entries(rawSession));
    expect(receipt.operationFingerprint).toBe(requestFingerprint);
    expect(session.expiresAtMs).toBe(firstSession.expiresAtMs);
...
test('ECDSA-only receipt reconstruction accepts absent NEAR state and rejects malformed present state', async () => {
  ...
  try {
    ...
    const receipt = Object.fromEntries(Object.entries(rawReceipt));
    const rawCommitted = receipt.committed;
    if (rawCommitted === null || typeof rawCommitted !== 'object' || Array.isArray(rawCommitted)) {
      throw new Error('ECDSA-only completion receipt is missing');
    }
    const committed = Object.fromEntries(Object.entries(rawCommitted));
    expect('nearProvisioning' in committed).toBe(false);

    committed.nearProvisioning = { status: 'near_ready' };
    receipt.committed = committed;
    completionRecord.receipt = receipt;
    await database
      .prepare(
...
test('V2 registration parser rejects mismatched identity, signing capability, material, and family', async () => {
  ...
  try {
    ...
    const signingIndex = exportOnlyWalletSession.capabilitySubjects.findIndex((subject) => {
      if (subject === null || typeof subject !== 'object' || Array.isArray(subject)) return false;
      return Object.fromEntries(Object.entries(subject)).kind === 'sign';
    });
    ...
    exportOnlyWalletSession.capabilitySubjects = exportOnlyWalletSession.capabilitySubjects.map(
      (subject, index) => {
        ...
        return { ...Object.fromEntries(Object.entries(subject)), kind: 'export_keys' };
      },
    );
    ...
    const existingSigningSubject = extraFamilyWalletSession.capabilitySubjects.find((subject) => {
      if (subject === null || typeof subject !== 'object' || Array.isArray(subject)) return false;
      return Object.fromEntries(Object.entries(subject)).kind === 'sign';
    });
    if (
      existingSigningSubject === null ||
      typeof existingSigningSubject !== 'object' ||
      Array.isArray(existingSigningSubject)
    ) {
      throw new Error('fixture signing capability is missing');
    }
    const existingSigningSubjectRecord = Object.fromEntries(Object.entries(existingSigningSubject));
    extraFamilyWalletSession.capabilitySubjects = [
      ...extraFamilyWalletSession.capabilitySubjects,
      { ...existingSigningSubjectRecord, keyFamily: 'ed25519' },
    ];
...
test('Email OTP + Ed25519-only: enrollment persists with the pending wallet, before any signer', async () => {
  ...
  try {
    ...
    const malformedActivateRequest = {
      ...activateRequest,
      idempotencyKey: 'ed25519-otp-extra-field',
      emailOtpEnrollment: {
        ...activateRequest.emailOtpEnrollment,
        unexpectedField: 'rejected',
      },
    };
    const malformedActivation =
      await service.walletRegistration.activateWalletRegistration(malformedActivateRequest);
    expect(malformedActivation).toMatchObject({ ok: false, code: 'invalid_body' });
