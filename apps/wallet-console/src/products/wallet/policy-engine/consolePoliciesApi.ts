...
export async function deleteDashboardPolicy(input: {
  ...
  const policyId = String(input.policyId || '').trim();
  if (!policyId) throw new Error('Policy id is required');
  const environments = await listPolicyRuntimeEnvironments(policyId);
  const base = requireConsoleBaseUrl();
  ...
  if (body?.removed === true) await publishPolicyRuntimeEnvironments(environments);
  return {
...
export async function publishDashboardPolicy(input: {
  ...
  if (policy.kind === 'TRANSACTION') {
    await publishPolicyRuntimeEnvironments(await listPolicyRuntimeEnvironments(policy.id));
  }
  return policy;
}
...
function assignmentIncludesEnvironment(
  assignment: DashboardConsolePolicyAssignment,
...
async function listPolicyRuntimeEnvironments(
  ...
  for (const assignment of assignments) {
    if (assignment.policyId === policyId) policyAssignments.push(assignment);
  }
  if (policyAssignments.length === 0) return [];
  const environments = await listDashboardEnvironments({ status: 'ACTIVE' });
  const affected: DashboardConsoleEnvironment[] = [];
  for (const environment of environments) {
    for (const assignment of policyAssignments) {
      if (assignmentIncludesEnvironment(assignment, environment)) {
        affected.push(environment);
        break;
      }
    }
  }
