...
test('D1 registration commit writes the founding authority and V2 method atomically', async () => {
  ...
  try {
    ...
    const reorderedAuthority: ActiveWalletAuthorityV1 = {
      activatedAtMs: founding.authority.activatedAtMs,
      state: founding.authority.state,
      updatedAtMs: founding.authority.updatedAtMs,
      createdAtMs: founding.authority.createdAtMs,
      revocationEpoch: founding.authority.revocationEpoch,
      authorityDigestB64u: founding.authority.authorityDigestB64u,
      signerActivationSetDigestB64u: founding.authority.signerActivationSetDigestB64u,
      signerActivations: founding.authority.signerActivations,
      permissions: founding.authority.permissions,
      provenance: founding.authority.provenance,
      principal: founding.authority.principal,
      walletId: founding.authority.walletId,
      authorityId: founding.authority.authorityId,
      kind: founding.authority.kind,
    };
    const reorderedAuthMethod: Extract<
      WalletAuthMethodRecordV2,
      { readonly kind: 'passkey'; readonly status: 'active' }
    > = {
      activatedAtMs: founding.authMethod.activatedAtMs,
      counter: founding.authMethod.counter,
      credentialPublicKeyB64u: founding.authMethod.credentialPublicKeyB64u,
      credentialIdB64u: founding.authMethod.credentialIdB64u,
      rpId: founding.authMethod.rpId,
      status: founding.authMethod.status,
      kind: founding.authMethod.kind,
      updatedAtMs: founding.authMethod.updatedAtMs,
      createdAtMs: founding.authMethod.createdAtMs,
      walletAuthorityId: founding.authMethod.walletAuthorityId,
      walletId: founding.authMethod.walletId,
      walletAuthMethodId: founding.authMethod.walletAuthMethodId,
      version: founding.authMethod.version,
    };
    /* D1 rows and callers may use different object insertion orders. */
    await store.commit({
      ...commitInput(),
      foundingAuthority: reorderedAuthority,
      foundingAuthMethod: reorderedAuthMethod,
    });
    ...
    /* A timestamp change is a meaningful replay difference even though all
       identity fields remain otherwise equivalent. */
