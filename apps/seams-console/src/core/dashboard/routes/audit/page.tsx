...
export type AuditApprovalDirectoryEntry = {
  id: string;
...
type AuditLogsPageProps = {
  approvalIntegration?: AuditApprovalIntegration;
};
...
export function AuditLogsPage({ approvalIntegration }: AuditLogsPageProps): React.JSX.Element {
  ...
  const [approvalDirectory, setApprovalDirectory] = React.useState<
    Record<string, AuditApprovalDirectoryEntry>
  >({});
  ...
  React.useEffect(() => {
    ...
    approvalIntegration
      ...
      .then((approvals) => {
        if (cancelled) return;
        const nextDirectory: Record<string, AuditApprovalDirectoryEntry> = {};
      ...
      >
        <div className="dashboard-audit-toolbar">
          ...
          <button
            ...
            title={`Download the ${events.length} loaded events across all table pages. Narrow the filters to export a different set.`}
          >
