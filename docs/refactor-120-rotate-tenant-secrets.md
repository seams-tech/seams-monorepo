...
Release status updated: September 10, 2026

Checklist reviewed: September 10, 2026. All 26 active implementation tasks are
already marked complete. The single release follow-up remains open; the
historical reference checklists retain their point-in-time state.
...
and staging operating path are complete. At the September 5 closure, the
previously released runtime tree was on `main`; the manual refresh throttle was
implemented and locally verified, with deployment still pending.

The following deployment evidence records the September 5 closure state.
Later backend/frontend deployments succeeded on September 8–9, as recorded in
[R121's execution checklist](./refactor-121-remaining-steps.md). Those deployments
do not establish the full staging/production and both-curve proof below.
...
The release follow-up is to verify the complete public Gateway/Console/frontend
and runtime rollout, including the one-hour manual refresh throttle, and record
fresh ECDSA and Ed25519 registration/signing proof. The reset requires no wallet
...
evidence follow Refactor 121's current contracts. Those contracts preserve the
preceding algebra and keep recovery-share decryption and import role-local.
Public verification may inspect both encrypted packages without opening either
share. Dashboard archives follow R121's governance policy: separate holder ZIPs
for two-person recovery and a combined encrypted-package ZIP for single-owner
recovery.
...
R120 supplies the deletion state types and evidence contracts described here.
R121 governs delivery of explicit source-lineage retirement and service-held
recovery-package destruction. Its current scope and acceptance evidence
determine completion of those operations.
...
operational shares, downloading tenant recovery artifacts, and restoring each
role into an empty replacement of the same logical tenant root. R121 is the
newer plan and takes precedence over R120's product and delivery assumptions.
Its [remaining steps](./refactor-121-remaining-steps.md) own current delivery
status; R120's completion evidence retains its recorded scope.

R121's current delivery bundles offline verification, session start/resume,
manifest registration, and one holder import into one CLI command. It retains
destination/bootstrap authentication, empty-destination enforcement, and
explicit CLI activation. Imports preserve the exact logical root identity.
The [same-site recovery design](./refactor-121-same-site-restore.md) is deferred;
its browser approval, active-root preflight, routing, and dashboard activation
are outside the current delivery.
...
### Closure evidence — September 5, 2026
...
- [ ] Verify the committed R120 behavior, including the hourly refresh throttle,
      across the complete staging and production Gateway/Console/frontend and
      runtime lanes; complete any missing rollout and prove fresh ECDSA and
      Ed25519 registration and signing. R121 records successful September 8–9
      backend/frontend deployments; the full combined proof remains unrecorded.
...
- [x] Exercise creation, refresh, one-role restore, and retirement in staging
      ...
      exposes no fault-injection route. This proof covers epoch retirement;
      whole-root deletion and provider erasure remain outside its evidence.
      Source-lineage retirement and recovery-package destruction follow R121's
      current delivery scope and acceptance criteria.
