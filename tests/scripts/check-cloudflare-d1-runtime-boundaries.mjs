...
const sdkServerReadmePath = 'packages/wallet-console-server-ts/README.md';
...
const activeRouterApiTextPaths = [
  ...
  'packages/wallet-console-server-ts/README.md',
  ...
  'packages/wallet-console-server-ts/wrangler.d1-staging-console.toml.example',
  'packages/wallet-console-server-ts/wrangler.d1-staging-gateway.toml.example',
...
const d1LocalBackupRestoreDrillScript =
  'packages/wallet-console-server-ts/scripts/d1-local-backup-restore-drill.mjs';
const d1StagingManifestWriterScripts = [
  d1LocalBackupRestoreDrillScript,
  'packages/wallet-console-server-ts/scripts/d1-staging-fixture-import.mjs',
  'packages/wallet-console-server-ts/scripts/d1-staging-migrate.mjs',
  'packages/wallet-console-server-ts/scripts/d1-staging-r2-restore-drill.mjs',
  'packages/wallet-console-server-ts/scripts/d1-staging-reconciliation.mjs',
  'packages/wallet-console-server-ts/scripts/d1-staging-resource-inventory.mjs',
  'packages/wallet-console-server-ts/scripts/d1-staging-signer-custody.mjs',
  'packages/wallet-console-server-ts/scripts/d1-staging-smoke.mjs',
  'packages/wallet-console-server-ts/scripts/d1-staging-time-travel-bookmark.mjs',
];
const d1StagingSharedHelperPath =
  'packages/wallet-console-server-ts/scripts/d1-staging-config.mjs';
const d1StagingCliHelperScripts = listJavaScriptFiles(
  'packages/wallet-console-server-ts/scripts',
).filter(
  (relativePath) =>
...
function consoleStagingWorkerSignerCustodyViolations() {
  ...
  const forbidden = ['SIGNER_DB', 'THRESHOLD_STORE', 'createCloudflareD1ConsoleServiceBundle'];
  const violations = [];
  for (const token of forbidden) {
    if (source.includes(token)) {
...
function routerApiStagingWorkerSignerCustodyViolations() {
  ...
  const required = ['SIGNER_DB', 'resolveSponsoredEvmWorkerExecutionAdapter'];
  const violations = [];
...
function d1WorkerRouterApiHandlerLifetimeViolations() {
  ...
  if (localWorker.includes('localRouterApiEcdsaPoolFillLiveSessions')) {
    violations.push(
      `${cloudflareD1LocalDevWorkerPath}: owns ECDSA pool-fill live sessions outside the Durable Object`,
    );
  }
  ...
  if (
    localWorker.includes('createRouterAbEcdsaDerivationPoolFillLiveSessionStore') ||
  ...
  for (const [relativePath, source] of [
    ...
    if (source.includes('SigningWorkerPresignSession')) {
      violations.push(
        `${relativePath}: references live Router A/B ECDSA derivation WASM presign sessions outside the Durable Object`,
      );
    }
    if (source.includes('presignSession') && source.includes('JSON.stringify')) {
      violations.push(
        `${relativePath}: may serialize Router A/B ECDSA derivation live presign session state outside the Durable Object`,
      );
    }
  }
...
function authServiceLegacyRegistrationModeViolations() {
  ...
  for (const rule of forbiddenAuthServiceLegacyRegistrationModeTokens) {
    const found = 'pattern' in rule ? rule.pattern.test(source) : source.includes(rule.token);
    if (found) violations.push(`${authServicePath}: ${rule.message}`);
  }
  return violations.sort();
}
...
function authServiceRouterApiHarnessViolations() {
  ...
  for (const relativePath of deletedAuthServiceRouterApiHarnessPaths) {
    if (fs.existsSync(toAbsolutePath(relativePath))) {
      violations.push(
        `${relativePath}: obsolete AuthService-backed Router API route harness exists`,
      );
    }
  }
  ...
  for (const root of scannedRoots) {
    for (const relativePath of listTypeScriptLikeFiles(root)) {
      ...
      if (!allowedAuthServiceConstructorPaths.has(relativePath)) {
        violations.push(
          `${relativePath}: constructs AuthService outside an explicitly non-Router owner`,
        );
      }
    }
  }
...
function d1EmailOtpRecoveryGrantBindingViolations() {
  ...
  const functionMatch = source.match(/function emailOtpGrantBindingMismatch\([\s\S]*?\n}\n/);
  if (!functionMatch) {
    return [`${cloudflareD1EmailOtpRecoveryServicePath}: missing Email OTP grant binding parser`];
  }
  const bindingFunction = functionMatch[0];
  const violations = [];
  for (const token of ['sessionHash', 'appSessionVersion']) {
    if (bindingFunction.includes(token)) {
      violations.push(`${cloudflareD1EmailOtpRecoveryServicePath}: grant binding reads ${token}`);
    }
  }
  for (const token of [
    ...
    if (!bindingFunction.includes(token)) {
      violations.push(`${cloudflareD1EmailOtpRecoveryServicePath}: grant binding missing ${token}`);
    }
  }
...
function authServiceRouterApiLifecycleMethodViolations() {
  ...
  for (const pattern of forbiddenAuthServiceRouterApiLifecycleMethodPatterns) {
    if (pattern.test(source)) {
      violations.push(
        `${authServicePath}: contains removed Router API lifecycle method ${pattern}`,
      );
    }
  }
  return violations.sort();
...
function d1LocalDevRouterApiRoutePrefixViolations() {
  ...
  if (!source.includes("pathname.startsWith('/near/')")) {
    violations.push(
      `${cloudflareD1LocalDevWorkerPath}: does not forward /near/* Router API routes`,
    );
  }
  if (source.includes("pathname.startsWith('-internal/shared-ts/near/')")) {
    violations.push(
      `${cloudflareD1LocalDevWorkerPath}: forwards malformed shared-ts route prefix instead of /near/*`,
    );
  }
  return violations.sort();
}
...
function syncAccountSessionPolicyInputViolations() {
  ...
  for (const relativePath of [
    ...
    if (readSource(relativePath).includes('threshold_ed25519')) {
      violations.push(
        `${relativePath}: exposes the removed sync-account threshold session-policy input`,
      );
    }
  }
  return violations.sort();
...
test('concrete D1 staging Wrangler configs stay untracked', () => {
  ...
  expect(source).toContain('packages/wallet-console-server-ts/wrangler.d1-staging-console.toml');
  expect(source).toContain('packages/wallet-console-server-ts/wrangler.d1-staging-gateway.toml');
});
