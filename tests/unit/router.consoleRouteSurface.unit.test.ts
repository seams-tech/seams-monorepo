...
import { createConsoleRouter } from '../../packages/wallet-console-server-ts/src/router/express/createConsoleRouter';
import {
...
import {
  composeConsoleRouteDefinitions,
  createWalletConsoleRouteDefinitions,
} from '../../packages/wallet-console-server-ts/src/router/walletConsoleRouteDefinitions';
...
test.describe('console route surface wiring', () => {
  test('core and Wallet route declarations have separate ownership', () => {
    const core = createConsoleRouteDefinitions();
    const wallet = createWalletConsoleRouteDefinitions();

    expect(core.some((route) => route.path.startsWith('/console/wallets'))).toBe(false);
    expect(core.some((route) => route.path.startsWith('/console/tenant-root'))).toBe(false);
    expect(wallet.some((route) => route.path.startsWith('/console/wallets'))).toBe(true);
    expect(wallet.some((route) => route.path.startsWith('/console/tenant-root'))).toBe(true);
  });

  test('composition rejects duplicate route declarations', () => {
    const core = createConsoleRouteDefinitions();
    const duplicate = core[0];
    if (!duplicate) throw new Error('Expected at least one core Console route');
    expect(() => composeConsoleRouteDefinitions(core, [duplicate])).toThrow(
      /Duplicate Console route id/u,
    );
  });
