...
function parseArgs(args) {
  ...
  if (!database || !config || !migrationsDir) {
    throw new Error(
      'Usage: apply-remote-d1-migrations.mjs --database <binding> --config <path> --migrations-dir <path> [--expected-fingerprint <sha256>] [--environment <name>]',
    );
  }
  return {
    database,
...
function spawnWrangler(options, commandArgs, stdio) {
  const environmentArgs = options.environment ? ['--env', options.environment] : [];
  return spawnSync(
    'pnpm',
    [
      'exec',
