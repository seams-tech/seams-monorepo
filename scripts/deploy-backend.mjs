...
const WALLET_SERVER_ARTIFACT_MANIFEST = readWalletServerArtifactManifest();
const WALLET_CONSOLE_ROOT = path.join(REPOSITORY_ROOT, 'packages', 'wallet-console-server-ts');
const GATEWAY_ROOT = WALLET_CONSOLE_ROOT;
const RENDER_WALLET_SYSTEM_CONFIG = path.join(
  WALLET_CONSOLE_ROOT,
  'scripts',
  'render-d1-gateway-config.mjs',
);
const APPLY_WALLET_SYSTEM_MIGRATIONS = path.join(
  WALLET_CONSOLE_ROOT,
  'scripts',
  'apply-remote-d1-migrations.mjs',
);
...
function resolveWalletServerRoot() {
  try {
    ...
    throw new Error(
      `@seams/wallet-server must be installed for hosted deployment: ${formatError(error)}`,
    );
  }
}

function readWalletServerArtifactManifest() {
  const manifestPath = path.join(WALLET_SERVER_ROOT, 'artifact-manifest.json');
  let manifest;
  try {
    manifest = JSON.parse(fs.readFileSync(manifestPath, 'utf8'));
  } catch (error) {
    throw new Error(`Unable to read @seams/wallet-server artifact manifest: ${formatError(error)}`);
  }
  if (
    manifest?.schemaVersion !== 'seams_wallet_server_artifact_manifest_v1' ||
    manifest?.package?.name !== '@seams/wallet-server' ||
    typeof manifest?.package?.version !== 'string' ||
    manifest.package.version.length === 0
  ) {
    throw new Error('@seams/wallet-server has an invalid artifact manifest');
  }
  return manifest;
}
...
async function main(args) {
  ...
  switch (options.operation) {
    case 'plan':
      printPlan(lane, options.component);
      return;
    case 'build':
      buildBackend(lane, options.component);
      return;
    case 'preflight':
      preflightBackend(lane, options.component);
      return;
    case 'migrate':
      migrateBackend(lane, options.component);
      return;
    case 'deploy':
      deployBackend(lane, options.component);
      return;
    case 'smoke':
      await smokeBackend(lane, options.component);
      return;
    default:
...
function parseArguments(args) {
  ...
  } else if (['plan', 'build', 'migrate', 'smoke'].includes(operation)) {
    if (!['console', 'wallet-system'].includes(options.component)) {
      throw new Error(`--component must be console or wallet-system for ${operation}`);
    }
...
function printPlan(lane, component) {
  const provisioningLines = provisioningPlanLines(lane);
  const header = [
    `Backend deployment plan: ${component} / ${lane.id}`,
  ...
  if (component === 'console') {
    process.stdout.write(
      `${[
        ...
        '',
        'Order:',
        '  1. build the Console Worker artifact',
        `  2. migrate ${lane.resources.gateway.consoleD1Name} (console D1)`,
        '  3. deploy Console',
        '  4. smoke Console readiness and CORS',
      ].join('\n')}\n`,
    );
    return;
  }
  const lines = [
    ...
    // Not implemented: every lane in deployment/wallet-system/targets.json is already
    // provisioned with a Router. Deployment-only sequencing either way; no
    // runtime compatibility path is required.
    '  4. deploy tenant-root-control-plane after the three workers',
    '  5. deploy router after the control plane and all three workers complete',
    '  6. deploy wallet-runtime after router',
    '  7. deploy gateway after router and wallet-runtime',
...
function buildBackend(lane, component) {
  process.stdout.write(`Building ${component} backend lane ${lane.id} (${lane.network})\n`);
  ...
  if (component === 'console') {
    buildConsoleArtifact(lane);
    return;
  }
  buildWalletSystemArtifacts(lane);
}

function buildConsoleArtifact(lane) {
...
function buildWalletSystemArtifacts(lane) {
  stageRouterRuntimeArtifact();
  runCommand('pnpm', ['-C', 'packages/wallet-console-server-ts', 'run', 'd1:local:ensure-wasm']);
...
function resolveRouterRuntimeRoot() {
  for (const candidate of [RELEASE_ROUTER_ROOT, PACKAGED_ROUTER_ROOT]) {
...
function componentRuntimeRequirements(lane, component) {
  switch (component) {
    ...
    case 'tenant-root-control-plane':
      return [
        ...(lane.id === 'production-mainnet' || lane.id === 'staging-testnet'
          ? ['TENANT_ROOT_RECOVERY_TRUST_BUNDLE_JSON', 'TENANT_ROOT_RECOVERY_TRUST_SNAPSHOT_JSON']
          : []),
    ...
    case 'console':
      return lane.id === 'production-mainnet' || lane.id === 'staging-testnet'
...
function deriverRuntimeRequirements(lane, role) {
  ...
  return [
    ...
    ...(managedBackup.kind === 'hpke'
      ? [managedBackup.publicEnvironmentName]
      : [`${prefix}_TENANT_ROOT_RECOVERY_KMS_KEY_RING`]),
    'TENANT_ROOT_CONTROL_PLANE_ISSUER_VERIFYING_KEYS_JSON',
    'TENANT_ROOT_CONTROL_PLANE_GRANT_AUTHORITY_VERIFYING_KEYS_JSON',
...
function migrateBackend(lane, component) {
  ...
  const migrations = {
    ...
    'wallet-system': {
      ...
      expectedFingerprint: walletServerMigration('signer').fingerprint,
    },
  };
  if (component === 'console') {
  ...
  const migrationArgs = [
    APPLY_WALLET_SYSTEM_MIGRATIONS,
    '--database',
    migration.database,
    '--config',
    migration.config,
...
export function assertEd25519RoleKeySet(secretName, keyIdName, keySetName, role, environment) {
  ...
  const activeField =
    role === 'deriver_a' ? 'active_deriver_a_signing_key_id' : 'active_deriver_b_signing_key_id';
...
function deployDeriver(lane, role) {
  ...
  if (managedBackup.kind === 'hpke') {
    ...
    args.push(
      ...
      '--var',
      `${prefix}_TENANT_ROOT_RECOVERY_KMS_KEY_RING:${requireEnvironmentValue(`${prefix}_TENANT_ROOT_RECOVERY_KMS_KEY_RING`)}`,
    );
  }
  runRouterCommand(args);
}
...
function deployTenantRootControlPlane(lane) {
  ...
  if (lane.id === 'production-mainnet' || lane.id === 'staging-testnet') {
    for (const name of [
      'TENANT_ROOT_RECOVERY_TRUST_BUNDLE_JSON',
      'TENANT_ROOT_RECOVERY_TRUST_SNAPSHOT_JSON',
    ]) {
      args.push('--var', `${name}:${requireEnvironmentValue(name)}`);
    }
  }
  runRouterCommand(args);
}
...
function deployConsole(lane) {
  ...
  runCommand('node', ['scripts/write-console-secrets-file.mjs', '--output', consoleSecrets], {
    cwd: GATEWAY_ROOT,
...
function migratePrivateD1(resource, configPath, component) {
  const deployment = PRIVATE_D1_DEPLOYMENTS[component];
  const migrationName = {
    'signing-worker': 'signing_worker',
    'deriver-a': 'deriver_a',
    'deriver-b': 'deriver_b',
  }[component];
  const migration = walletServerMigration(migrationName);
  const args = [
    APPLY_WALLET_SYSTEM_MIGRATIONS,
    '--database',
    deployment.binding,
  ...
  if (resource.deploymentEnvironment.kind === 'named') {
    args.push('--environment', resource.deploymentEnvironment.name);
  }
  runCommand('node', args, { cwd: GATEWAY_ROOT });
}

function walletServerMigration(name) {
  const migration = WALLET_SERVER_ARTIFACT_MANIFEST.migrations?.[name];
  if (
    !migration ||
    typeof migration.directory !== 'string' ||
    typeof migration.head !== 'string' ||
    !/^[0-9a-f]{64}$/u.test(migration.fingerprint)
  ) {
    throw new Error(`@seams/wallet-server artifact manifest is missing migration set ${name}`);
  }
  return migration;
}
...
async function smokeBackend(lane, component) {
  ...
  if (component === 'wallet-system') {
    ...
    checks.push({
      name: 'Console /readyz',
      url: new URL('/console/readyz', consoleOrigin).toString(),
    });
    checks.push({
...
function renderConsoleConfig(laneId, outputPath) {
  runCommand(
    'node',
    [RENDER_WALLET_SYSTEM_CONFIG, '--lane', laneId, '--worker', 'console', '--output', outputPath],
    { cwd: GATEWAY_ROOT },
  );
}
