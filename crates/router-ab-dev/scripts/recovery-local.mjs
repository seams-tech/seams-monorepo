...
function ensureWorkers(directory) {
  const receiptPath = path.join(
    repoRoot,
    'crates/router-ab-cloudflare/build/local-build-receipt.json',
  );
  const receipt = existsSync(receiptPath) ? JSON.parse(readFileSync(receiptPath, 'utf8')) : null;
  if (receipt?.source_digest === strictWorkerSourceDigest(repoRoot)) return;
  console.log('Building updated recovery Workers. This first build can take several minutes...');
  runLogged(
    process.execPath,
    [path.join(repoRoot, 'crates/router-ab-dev/scripts/dev-local-workers.mjs'), '--build-only'],
    path.join(directory, 'worker-build.log'),
  );
}
