...
test.describe('Wallet Session authorization status client', () => {
  ...
  test('rejects object instances and arrays at the JSON boundary', async () => {
    const { identity } = await activeStatusResponseBody();

    expect(parseExactWalletSessionStatusResponse(new Date(), identity)).toBeNull();
    expect(parseExactWalletSessionStatusResponse([], identity)).toBeNull();
  });
});
