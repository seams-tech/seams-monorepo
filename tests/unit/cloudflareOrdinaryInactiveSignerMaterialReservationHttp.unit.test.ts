...
test('deactivation endpoint returns the parsed terminal domain result', async () => {
  const calls: RecordedRequestV1[] = [];
  const materialActivation = buildOrdinaryMaterialActivationFixture('deactivation-http');
  const endpoint = createCloudflareOrdinaryInactiveSignerMaterialDeactivationEndpointV1({
    fetch: createRecordingFetch(calls, [
      {
        state: 'revoked',
        reservation_id: 'server-reservation-deactivation',
  ...
  await expect(
    endpoint.deactivateInactiveEd25519SignerMaterialV1({ materialActivation }),
  ).resolves.toEqual({
    kind: 'ordinary_ed25519_signer_material_deactivation_v1',
    keyFamily: 'ed25519',
    state: 'revoked',
    materialActivation,
    serverMaterialReservationId: 'server-reservation-deactivation',
