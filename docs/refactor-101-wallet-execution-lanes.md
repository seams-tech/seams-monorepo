...
refresh follow the current curve-specific Router A/B protocols; device linking
remains R103 work; delegated spending remains R104 work.
...
- the current [Router A/B protocols](./router-ab/protocol.md) for signing
  material provisioning, refresh, activation, and revocation;
...
Refactor 102 is superseded. [Refactor 120](./refactor-120-rotate-tenant-secrets.md)
owns tenant-root creation and operational-share refresh;
[Refactor 121](./refactor-121-tenant-derivation-root-security.md) owns root
recovery controls and exact-root disaster recovery. Neither operation changes
active signing-lane material. Current production protocol types and encoders
define the retained curve-specific lane contracts.
...
an independently provisioned lane requires them.
...
ports and projection adapters over that authority. Lane provisioning may extend
persistence when a wallet key can own more than one independently provisioned lane.

The current curve-specific Router A/B protocols own jobs, material delivery,
activation, refresh, and cryptographic revocation receipts. Gateway D1 owns
product ceremony outcomes, Wallet Session authorization, quotas, authorized operations,
...
normal-signing flows, create lane-provisioning jobs, or enable linked/delegated
branches.
...
encodings. It exposes narrow adapter ports for signing protocols and device or
delegated admission.
...
The current Router A/B protocols own those operations and receipts. R103 enables
linked-device admission. R104 enables delegated admission and replaces the remaining dormant agent
...
- Existing owner signer rows project through
  ...
  provisioned linked-device and delegated lanes. Provisioning reuses them.
...
- Cryptographic revocation receipt encoding and post-compromise share refresh
  belong to the current curve-specific signing protocols. R101 persists and
  checks the product revocation epoch and verified receipt digest exposed by the
  canonical signer record.
- Existing `wallet_signers` records remain the first R101 persistence source.
  New lane tables require a demonstrated multi-lane write path.
