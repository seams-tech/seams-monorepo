...
function substitutedReceipt(raw: unknown, substitution: string): unknown {
  const value = copyRecord(raw, 'receipt');
  switch (substitution) {
...
test('lane creation requires an owner source and refresh preserves the lane kind', () => {
  const vector = fixture();
  const linkedSource = copyRecord(vector.job, 'job');
  const linkedSourceRecord = copyNestedRecord(linkedSource, 'source');
  ...
  const changedKind = copyRecord(vector.job, 'job');
  const source = copyNestedRecord(changedKind, 'source');
