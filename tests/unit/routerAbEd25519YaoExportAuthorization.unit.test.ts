...
class StaticExportStateStore implements RouterAbEd25519YaoProductRegistrationPartitionedStateStoreV1 {
  ...
  async load(
    ...
    return {
      state: this.state,
      baseline: {
        sharedEncoding: encodeRouterAbEd25519YaoProductRegistrationPartitionRecordV1({
  ...
  async commit(
    ...
    return {
      kind: 'stored',
      sharedVersion: input.baseline.sharedVersion,
      ceremonyVersion: 'static-export-test',
      executionVersion: input.baseline.executionVersion,
    };
  }
