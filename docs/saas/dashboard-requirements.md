...
- Financial correctness: invoice balances, credits, and payment settlement states remain consistent and auditable. Successful refunds debit prepaid credit, and their persisted state must reflect provider settlement.
- Runtime snapshot contract for relay/runtime consumers uses full versioned per-environment snapshots. Console policy publication and deletion refresh every affected active environment before reporting success.
- Webhook endpoint selection updates its deep link; delivery responses from an earlier selection cannot replace the current endpoint’s rows.
- Gas sponsorship preserves exact NEAR method names through loading, editing, and publication.
- Derivation-root security shows an unprovisioned state when an environment has no active tenant root; upstream failures remain errors.
...
- Phase 2: policy simulation/versioning, gas sponsorship budgets through `GAS_SPONSORSHIP` policies, key export approvals, prepaid Stripe Checkout top-ups, pricing -> Stripe Checkout -> dashboard return flow.
...
- Audit logs export the currently loaded, filtered events as CSV across all table pages (up to 100 events); the button states this scope.
- Administrators with either `members.manage` or `projects.manage` can read organization memberships and invitations. Membership mutations retain their separate permissions.
