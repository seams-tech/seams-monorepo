...
const EXPECTED_STAGING_ORIGINS = Object.freeze({
  gateway: 'https://staging.api.wallet.seams.sh',
...
const webServer =
  REFACTOR93_STAGING_CONFIG.mode === 'live'
    ? {
        command:
          'pnpm -C .. build:sdk-full && pnpm -C .. exec vite --config tests/intended-app/vite.config.ts --host 127.0.0.1 --port 37994',
...
function readStagingOrigins(): typeof EXPECTED_STAGING_ORIGINS {
  const targetsPath = path.join(REPOSITORY_ROOT, 'deployment', 'wallet-system', 'targets.json');
  const parsed = JSON.parse(readFileSync(targetsPath, 'utf8')) as unknown;
  if (parsed === null || typeof parsed !== 'object' || Array.isArray(parsed)) {
    throw new Error('deployment/wallet-system/targets.json is missing staging origins');
  }
  const staging = Reflect.get(parsed, 'staging');
  if (staging === null || typeof staging !== 'object' || Array.isArray(staging)) {
    throw new Error('deployment/wallet-system/targets.json is missing staging origins');
  }
  const site = Reflect.get(staging, 'site');
  const lanes = Reflect.get(staging, 'lanes');
  if (
    site === null ||
    typeof site !== 'object' ||
    Array.isArray(site) ||
  ...
  const testnet = Reflect.get(lanes, 'testnet');
  if (testnet === null || typeof testnet !== 'object' || Array.isArray(testnet)) {
    throw new Error('deployment/wallet-system/targets.json is missing staging origins');
  }
  const gateway = Reflect.get(testnet, 'gatewayOrigin');
  const siteOrigin = Reflect.get(site, 'origin');
  const wallet = Reflect.get(testnet, 'walletOrigin');
  if (typeof gateway !== 'string' || typeof siteOrigin !== 'string' || typeof wallet !== 'string') {
    throw new Error('deployment/wallet-system/targets.json is missing staging origins');
  }
  const origins = {
    gateway,
