...
test('deduplicates equivalent capabilities regardless of insertion order and rejects drift', async () => {
  const manifest = ecdsaCapabilityHydrationLookupFixture().active.manifest;
  const publicFacts = manifest.durableMaterial.roleLocalPublicFacts;
  const publicCapability = publicFacts.publicCapability;
  const reorderedPublicCapability = {
    ...publicCapability,
    signer_set: {
      ...publicCapability.signer_set,
      selected_server: {
        recipient_encryption_key:
          publicCapability.signer_set.selected_server.recipient_encryption_key,
        key_epoch: publicCapability.signer_set.selected_server.key_epoch,
        server_id: publicCapability.signer_set.selected_server.server_id,
      },
    },
    kind: publicCapability.kind,
  };
  const reorderedManifest = {
    ...manifest,
    durableMaterial: {
      ...manifest.durableMaterial,
      roleLocalPublicFacts: {
        ...publicFacts,
        publicCapability: reorderedPublicCapability,
      },
    },
  };
  const resolve = (manifests: readonly (typeof manifest)[]) =>
    resolveEmailOtpExistingEcdsaKey({
      ...
      keyHandle: String(publicFacts.keyHandle),
      listActiveEcdsaCapabilityManifestsForWallet: async () => manifests,
    });

  await expect(resolve([manifest, reorderedManifest])).resolves.not.toBeNull();

  const conflictingManifest = {
    ...reorderedManifest,
    durableMaterial: {
      ...reorderedManifest.durableMaterial,
      roleLocalPublicFacts: {
        ...reorderedManifest.durableMaterial.roleLocalPublicFacts,
        publicCapability: {
          ...reorderedPublicCapability,
          client_id: `${reorderedPublicCapability.client_id}:drift`,
        },
      },
    },
  };
  await expect(resolve([manifest, conflictingManifest])).rejects.toThrow(
    'conflicting persisted role-local material',
  );
});
