...
export type ConsoleRouteRequirement =
  ...
  | 'billing.manage'
  | 'platform.support'
  // Tenant derivation-root mutations additionally require server-issued
  // fresh step-up evidence, which session claims cannot express. The
  // requirement is in the route table so a policy test can assert that every
  // such route carries it.
  | 'project.edit.step_up'
  | 'owner.step_up'
  // The tenant-root refresh route keeps its existing `projects.manage`
  // audience and gains step-up. Reusing `project.edit.step_up` here would
  // widen it to any project editor, which is a different route, not a
  // stricter one.
  | 'projects.manage.step_up';
...
export type ConsoleRouteTuple = readonly [id: string, method: ConsoleRouteMethod, path: string];
...
const PROJECT_EDIT_ROUTES: readonly ConsoleRouteTuple[] = [
  ...
  ['console_webhooks_test', 'POST', '/console/webhooks/:id/test'],
...
export function defineConsoleRoutes(
  requirement: ConsoleRouteRequirement,
  tuples: readonly ConsoleRouteTuple[],
): readonly ConsoleRouteDefinition[] {
  const definitions: ConsoleRouteDefinition[] = [];
  appendRoutes(definitions, requirement, tuples);
...
/**
 * Returns whether a route needs server-issued fresh step-up evidence.
 *
 * Session claims cannot prove step-up, so the route policy grants the coarse
 * access and the handler must additionally verify the evidence. This predicate
 * is what makes that obligation visible and testable.
 */
export function consoleRouteRequiresFreshStepUpV1(route: ConsoleRouteDefinition): boolean {
  return (
    route.auth.requirement === 'project.edit.step_up' ||
    route.auth.requirement === 'owner.step_up' ||
    route.auth.requirement === 'projects.manage.step_up'
  );
}
