...
- Complete Wallet-system environment preparation: `pnpm wallet-system:deploy:env-prepare -- --lane staging-testnet --repo seams-tech/seams-monorepo`
- Complete Console environment preparation: `pnpm console:deploy:env-prepare -- --lane staging-testnet`
...
`wallet-system:deploy:env-prepare` generates the deployment identities, internal
...
secrets. `console:deploy:env-prepare` separately generates only Console-owned
session, invitation, webhook, billing, OAuth, and grant-authority configuration.
The Router or self-host relay serves the public keyset for SDK prefetch.
