...
function normalizeIdentity(identity: TenantRootIdentityV1): TenantRootIdentityV1 {
  const decoded = decodeTenantRootIdentityWireV1(identity);
  if (!decoded.ok) {
    throw new TenantRootCreationGrantStoreError(
      'invalid_input',
      'tenant-root identity is not canonical',
    );
  }
...
function parseRecord(row: D1Row): TenantRootCreationGrantRecordV1 {
  const identity = decodeTenantRootIdentityWireV1({
    orgId: row.org_id,
    projectId: row.project_id,
    envId: row.env_id,
    signingRootId: row.signing_root_id,
    signingRootVersion: row.signing_root_version,
  });
  if (!identity.ok) {
    throw new TenantRootCreationGrantStoreError(
      'invalid_record',
      'tenant-root creation grant row has a non-canonical identity',
    );
  }
  const base = {
