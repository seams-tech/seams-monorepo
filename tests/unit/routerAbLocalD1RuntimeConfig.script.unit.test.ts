...
function createRuntimeFixture(): RuntimeFixture {
  ...
  writeEnv(root, '.env.local', { SEAMS_LOCAL_CONSOLE_ORG_ID: 'org_test12345678' });
...
function prepareStrictRuntime(fixture: RuntimeFixture) {
  const d1Runtime = prepareRouterAbD1LocalRuntimeConfig({
    ...
    localConsoleProjectId: 'customer-project',
    localConsoleEnvironmentId: 'customer-project:dev',
  });
...
test('local recovery KMS settings preserve operational HPKE and keep credentials in role secrets', () => {
  const fixture = createRuntimeFixture();
  for (const role of ['a', 'b']) {
    const suffix = role.toUpperCase();
    const envPath = path.join(fixture.root, `.env.router-ab.deriver-${role}.local`);
    writeFileSync(
      envPath,
      readFileSync(envPath, 'utf8') +
        `DERIVER_${suffix}_TENANT_ROOT_RECOVERY_KMS_KEY_RING=projects/test/locations/global/keyRings/recovery-${role}\n` +
        `DERIVER_${suffix}_TENANT_ROOT_MANAGED_BACKUP_GOOGLE_CREDENTIALS_JSON=fixture-credentials-${role}\n`,
    );
  }
  prepareStrictRuntime(fixture);
  for (const role of ['a', 'b']) {
    const suffix = role.toUpperCase();
    const root = path.join(fixture.root, '.runtime/router-ab-strict');
    const config = readFileSync(path.join(root, `wrangler.deriver-${role}.toml`), 'utf8');
    const secrets = readFileSync(path.join(root, `.dev.vars.deriver-${role}`), 'utf8');
    expect(config).toContain(`keyRings/recovery-${role}`);
    expect(config).toContain(
      `DERIVER_${suffix}_TENANT_ROOT_MANAGED_BACKUP_GOOGLE_CREDENTIALS_JSON_BINDING`,
    );
    expect(config).toContain(`DERIVER_${suffix}_TENANT_ROOT_MANAGED_BACKUP_HPKE_PUBLIC_KEY`);
    expect(config).not.toContain(`fixture-credentials-${role}`);
    expect(secrets).toContain(`fixture-credentials-${role}`);
  }
});
