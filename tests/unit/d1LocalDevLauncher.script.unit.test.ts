...
async function loadD1LocalDevLauncherModule(): Promise<D1LocalDevLauncherModule> {
  const modulePath = path.join(
    repoRoot(),
    'packages/wallet-console-server-ts/scripts/d1-local-dev.mjs',
  );
...
test('D1 local dev launcher honors process isolation without an explicit env argument', async () => {
  const module = await launcherModulePromise;
  const tree = createTempLocalDevTree();
  writeRootLocalEnv(tree.root);
  const previous = process.env.SEAMS_D1_LOCAL_SKIP_ENV_FILE;
  process.env.SEAMS_D1_LOCAL_SKIP_ENV_FILE = '1';
  try {
    const command = module.buildD1LocalDevWranglerArgs({
      repoRoot: tree.root,
      packageRoot: tree.consolePackageRoot,
    ...
    if (previous === undefined) delete process.env.SEAMS_D1_LOCAL_SKIP_ENV_FILE;
    else process.env.SEAMS_D1_LOCAL_SKIP_ENV_FILE = previous;
  }
});
