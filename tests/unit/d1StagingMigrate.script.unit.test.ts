...
test('D1 staging migration plan records migration hashes and noninteractive apply commands', async () => {
  ...
  expect(plan.commands[1].command).toContain(
    'CI=true pnpm --dir packages/wallet-console-server-ts exec wrangler',
  );
  expect(plan.commands[1].command).toContain(
...
test('D1 staging migration rejects failed remote migration commands', async () => {
  ...
  expect(() =>
    ...
    /Command failed: pnpm --dir packages\/wallet-console-server-ts exec wrangler d1 migrations list/,
  );
});
