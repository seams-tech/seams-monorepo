...
function main() {
  ...
  const secrets = readRequiredSecrets(consoleSecretNames(lane));
...
function readOutputPath(args) {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    throw new Error('usage: write-console-secrets-file.mjs --output <path>');
  }
...
function requireProvisionedLane(laneId, provisioning) {
  if (provisioning.kind !== 'provisioned') {
    throw new Error(`lane ${laneId} is pending provisioning; Console secrets cannot be written`);
  }
}
