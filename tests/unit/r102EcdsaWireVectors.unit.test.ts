...
function copyRecord(value: unknown, label: string): Record<string, unknown> {
  return Object.fromEntries(Object.entries(record(value, label)));
}

function copyNestedRecord(value: Record<string, unknown>, key: string): Record<string, unknown> {
  const copied = copyRecord(value[key], key);
  value[key] = copied;
  return copied;
}
...
function substitutedJob(raw: unknown, substitution: string): unknown {
  const value = copyRecord(raw, 'job');
  const source = copyNestedRecord(value, 'source');
  const target = copyNestedRecord(value, 'target');
  const holder = copyNestedRecord(value, 'targetHolder');
  const worker = copyNestedRecord(value, 'targetSigningWorker');
  const authorization = copyNestedRecord(value, 'authorization');
  const targetCapability = copyNestedRecord(value, 'targetCapability');
  ...
  switch (substitution) {
    ...
    case 'thresholdSessionOrder': {
      ...
      targetCapability.orderedThresholdSessions = [...sessions].reverse();
      break;
    }
    default:
