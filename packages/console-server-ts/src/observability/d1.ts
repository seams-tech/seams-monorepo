...
import type { ConsoleObservabilityRequestMetricPolicy } from './policy';
...
interface D1ConsoleObservabilityIngestionState {
  ...
  readonly acceptedSources: ReadonlySet<ConsoleObservabilitySource>;
  readonly requestMetricPolicies: readonly ConsoleObservabilityRequestMetricPolicy[] | undefined;
}
...
export interface D1ConsoleObservabilitySchemaOptions {
  readonly database: D1DatabaseLike;
  readonly additionalSources?: readonly ConsoleObservabilitySource[];
}

export interface D1ConsoleObservabilityServiceOptions extends Pick<
  InMemoryConsoleObservabilityServiceOptions,
  'now'
> {
  readonly database: D1DatabaseLike;
...
export interface D1ConsoleObservabilityIngestionServiceOptions extends Pick<
  InMemoryConsoleObservabilityServiceOptions,
  'now'
> {
  readonly database: D1DatabaseLike;
  ...
  readonly maxEventsPerMinute?: number;
  readonly additionalSources?: readonly ConsoleObservabilitySource[];
  readonly requestMetricPolicies?: readonly ConsoleObservabilityRequestMetricPolicy[];
}
...
export async function ensureConsoleObservabilityD1Schema(
  ...
  const sourcesSql = Array.from(
    new Set([...CONSOLE_OBSERVABILITY_SOURCE_SET, ...(options.additionalSources || [])]),
  )
    .map((source) => `'${source}'`)
    .join(', ');
  for (const statement of CONSOLE_OBSERVABILITY_D1_SCHEMA_SQL) {
    await options.database.exec(
      formatD1ExecStatement(statement.replace(CONSOLE_OBSERVABILITY_SOURCES_SQL, sourcesSql)),
    );
  }
}
...
export async function createD1ConsoleObservabilityIngestionService(
  ...
  const state: D1ConsoleObservabilityIngestionState = {
    ...
    requestMetricPolicies: options.requestMetricPolicies,
  };
  if (options.ensureSchema !== false) {
    await ensureConsoleObservabilityD1Schema({
...
function ensureSource(
  raw: unknown,
  acceptedSources: ReadonlySet<ConsoleObservabilitySource>,
): ConsoleObservabilitySource {
  const value = normalizeString(raw).toUpperCase() as ConsoleObservabilitySource;
  if (!acceptedSources.has(value)) {
...
function normalizeEnvelopeForInsert(
  ...
  const source = ensureSource(input.source, acceptedSources);
...
function eventWindowWhere(input: {
  ...
  return buildProjectEnvironmentWhere({
    initialClauses: ['namespace = ?', 'org_id = ?', 'created_at_ms >= ?', 'created_at_ms <= ?'],
...
function rollupWindowWhere(input: {
  ...
  return buildProjectEnvironmentWhere({
    initialClauses: ['namespace = ?', 'org_id = ?', 'window_start_ms >= ?', 'window_start_ms <= ?'],
    initialValues: [input.namespace, input.orgId, input.window.fromMs, input.window.toMs],
    projectId: input.request.projectId,
    environmentId: input.request.environmentId,
  });
...
async function observeRequestMetric(input: {
  ...
  readonly policies?: readonly ConsoleObservabilityRequestMetricPolicy[];
}): Promise<void> {
  const normalized = normalizeConsoleObservabilityRequestMetricForInsert(
    input.metric,
    input.policies,
  );
...
class D1ConsoleObservabilityIngestionServiceImpl implements ConsoleObservabilityIngestionD1Service {
  readonly [CONSOLE_OBSERVABILITY_INGESTION_D1_RUNTIME]: ConsoleObservabilityD1Runtime;
  ...
  async appendEvents(
    ...
    for (const event of events) {
      ...
      normalizedEvents.push(
        normalizeEnvelopeForInsert(event, this.state.redactionPolicy, this.state.acceptedSources),
      );
    }
  ...
  async observeRequestMetric(
    ...
    await observeRequestMetric({
      ...
      orgId,
      metric,
      policies: this.state.requestMetricPolicies,
    });
  }
}
...
class D1ConsoleObservabilityServiceImpl implements ConsoleObservabilityD1Service {
  ...
  async getSummary(
    ...
    const orgId = ensureRequiredString('ctx.orgId', ctx.orgId);
    const window = resolveBoundedQueryWindow(
      request,
      this.state.now(),
      this.state.queryMaxWindowMs,
    );
  ...
  async listEvents(
    ...
    const clauses = ['namespace = ?', 'org_id = ?', 'created_at_ms >= ?', 'created_at_ms <= ?'];
  ...
  async getTimeseries(
    ...
    for (const row of rows) {
      ...
      const current = countsByStart.get(key) || {
        errorCount: 0,
        requestCount: 0,
    ...
    for (
      let bucketStartMs = window.fromMs;
      bucketStartMs <= window.toMs;
