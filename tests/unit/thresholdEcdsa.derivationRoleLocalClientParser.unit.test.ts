...
import {
  parseThresholdEcdsaDerivationRoleLocalBootstrapValue,
...
test.describe('threshold ECDSA derivation role-local client parser', () => {
  ...
  test('rejects class instances at the bootstrap value boundary', () => {
    class HostileResponseValue {
      readonly formatVersion = 'ecdsa-derivation-role-local';
    }

    const hostile = Object.assign(new HostileResponseValue(), bootstrapValue());
    expect(() => parseThresholdEcdsaDerivationRoleLocalBootstrapValue(hostile)).toThrow(
