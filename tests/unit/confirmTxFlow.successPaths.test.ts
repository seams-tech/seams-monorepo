...
test.describe('confirmTxFlow – success paths', () => {
  ...
  test('LocalOnly: authorizeKeyExport returns credential (no prfOutput)', async ({
    ...
    const result = await page.evaluate(
      async ({ paths }) => {
        ...
        const request = {
          requestId: 'r1',
          type: types.UserConfirmationType.AUTHORIZE_KEY_EXPORT,
          summary: {},
          payload: {
  ...
  test('LocalOnly: authorizeKeyExport skips intermediate confirm UI in wallet-iframe host mode', async ({
    page,
  }) => {
    // Export Private Key UX in wallet-iframe host mode:
    // - For the initial AUTHORIZE_KEY_EXPORT step, we should proceed directly to the
