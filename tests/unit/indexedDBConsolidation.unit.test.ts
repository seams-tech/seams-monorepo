...
test.describe('IndexedDB consolidation', () => {
  ...
  test('wallet signer finalize rollback removes only the exact batch and restores selection', async ({
    ...
    const result = await page.evaluate(async () => {
      ...
      const signerActivations = [
        {
          ...
          signer: {
            ...
            signerSource: 'passkey_registration',
            metadata: {
              nearEd25519SigningKeyId: 'rollback-signing-key',
              rollbackMetadata: { source: 'rollback-test', sequence: 1 },
            },
          },
      ...
      const keyMaterials = [
        {
          ...
          signerId: 'ed25519:rollback-public',
          payload: {
            rollbackPayload: { source: 'rollback-test', sequence: 1 },
          },
          payloadEnvelope: {
            encVersion: 1,
            alg: 'AES-GCM',
            nonce: 'rollback-nonce',
            ciphertext: 'rollback-ciphertext',
            tag: 'rollback-tag',
            aad: {
              profileId: walletId,
              signerSlot: 2,
      ...
      const db = await manager.getDB();
      const signerRows = (await db.getAll(schemaNames.SEAMS_WALLET_STORES.walletSigners)) as Array<{
        wallet_id: string;
        record: { metadata?: Record<string, unknown> };
        [key: string]: unknown;
      }>;
      const signerRow = signerRows.find((row) => row.wallet_id === walletId);
      if (!signerRow) throw new Error('rollback test signer row is missing');
      const originalSignerRow = signerRow;
      await db.put(schemaNames.SEAMS_WALLET_STORES.walletSigners, {
        ...signerRow,
        record: {
          ...signerRow.record,
          metadata: {
            ...signerRow.record.metadata,
            rollbackMetadata: { source: 'rollback-test', sequence: 2 },
          },
        },
      });
      const signerMetadataMutationRejected = await repositories
        .rollbackWalletSignerFinalize(persisted.rollbackReceipt)
        .then(() => false)
        .catch((error: Error) =>
          String(error.message || '').includes('wallet signer rollback signer changed'),
        );
      await db.put(schemaNames.SEAMS_WALLET_STORES.walletSigners, originalSignerRow);

      const keyMaterialRows = (await db.getAll(
        schemaNames.SEAMS_WALLET_STORES.keyMaterial,
      )) as Array<{
        wallet_id: string;
        record: {
          payload?: Record<string, unknown>;
          payloadEnvelope?: { tag?: string };
        };
        [key: string]: unknown;
      }>;
      const keyMaterialRow = keyMaterialRows.find((row) => row.wallet_id === walletId);
      if (!keyMaterialRow) throw new Error('rollback test key material row is missing');
      const originalKeyMaterialRow = keyMaterialRow;
      await db.put(schemaNames.SEAMS_WALLET_STORES.keyMaterial, {
        ...keyMaterialRow,
        record: {
          ...keyMaterialRow.record,
          payload: {
            ...keyMaterialRow.record.payload,
            rollbackPayload: { source: 'rollback-test', sequence: 2 },
          },
        },
      });
      const keyMaterialPayloadMutationRejected = await repositories
        .rollbackWalletSignerFinalize(persisted.rollbackReceipt)
        .then(() => false)
        .catch((error: Error) =>
          String(error.message || '').includes('wallet signer rollback key material changed'),
        );
      await db.put(schemaNames.SEAMS_WALLET_STORES.keyMaterial, originalKeyMaterialRow);

      const payloadEnvelope = keyMaterialRow.record.payloadEnvelope;
      if (!payloadEnvelope) {
        throw new Error('rollback test key material envelope is missing');
      }
      const envelopeMutationRow = {
        ...keyMaterialRow,
        record: {
          ...keyMaterialRow.record,
          payloadEnvelope: {
            ...payloadEnvelope,
            tag: 'rollback-tag-changed',
          },
        },
      };
      await db.put(schemaNames.SEAMS_WALLET_STORES.keyMaterial, envelopeMutationRow);
      const keyMaterialEnvelopeMutationRejected = await repositories
        .rollbackWalletSignerFinalize(persisted.rollbackReceipt)
        .then(() => false)
        .catch((error: Error) =>
          String(error.message || '').includes('wallet signer rollback key material changed'),
      ...
      return {
        signerCountBefore,
        signerCountAfter,
        signerMetadataMutationRejected,
        keyMaterialPayloadMutationRejected,
        keyMaterialEnvelopeMutationRejected,
        keyMaterialAfter,
    ...
    expect(result).toEqual({
      signerCountBefore: 1,
      signerCountAfter: 0,
      signerMetadataMutationRejected: true,
      keyMaterialPayloadMutationRejected: true,
      keyMaterialEnvelopeMutationRejected: true,
  ...
  test('rejects active Wallet Session replay when only quotaId changes', async ({ page }) => {
    const fixture = await buildLocalAuthorityInstallationFixture();
    const pendingAuthority = fixture.input.authority;
    const activeAuthorityDraft = buildActiveWalletAuthorityV1({
      kind: pendingAuthority.kind,
    ...
    const conflictingQuotaId = unwrap(
      parseMpcWalletSigningQuotaId('quota:r103e-install-conflict'),
    );
    const conflictingWalletSession = parseActiveWalletSessionV1({
      kind: 'active_wallet_session_v1',
      ...
      authorizationId: activeWalletSession.authorizationId,
      quotaId: conflictingQuotaId,
    ...
    const result = await page.evaluate(
      async ({
        ...
        const repositoryModule =
          await import('/_test-sdk/esm/core/indexedDB/seamsWalletDB/repositories.js');
        const dbName = schemaNames.createSeamsTestWalletDbName(
          `authority_finalize_quota_${crypto.randomUUID()}`,
        );
        const manager = new managerModule.SeamsWalletDBManager();
        manager.setDbName(dbName);
        const repositories = new repositoryModule.SeamsWalletRepositories(manager);
        ...
        const finalized = await repositories.finalizeLocalAuthorityActivation({
          authority: activeAuthority,
          authMethod: activeAuthMethod,
          walletSession: activeWalletSession,
          operationCredential,
        ...
        try {
          await repositories.finalizeLocalAuthorityActivation({
            authority: activeAuthority,
            authMethod: activeAuthMethod,
            walletSession: conflictingWalletSession,
            operationCredential,
            expectedLockGeneration: fixture.input.expectedLockGeneration,
          });
        } catch (error) {
        ...
        return { finalized, conflictMessage };
      },
      {
        fixture,
        unlockedSelection,
    ...
    expect(result).toEqual({
      ...
      conflictMessage: 'active local authority replay conflicts with supplied Wallet Session',
    });
  });
