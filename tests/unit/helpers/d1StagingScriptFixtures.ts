...
export async function loadD1StagingScriptModule<T>(
...
export function walletConsolePackagePath(...segments: string[]): string {
  return path.join(D1_STAGING_REPO_ROOT, 'packages/wallet-console-server-ts', ...segments);
}
